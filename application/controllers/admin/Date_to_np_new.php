 ...
 * 
 */
class Date_to_np_new extends Admin_Controller 
{
	...
	public function index()
	{
		...
		$tables = array(
			...
			// 	)
			array(
				'table_name'=>'sales_vehicle_process',
				'fields' => array(
					array('eng_date' => 'booked_date', 'nep_date'=>'booked_date_np'),
					),
				)
			);
		...
		foreach ($tables as $table) {
			foreach($table['fields'] as $field){
				...
				$this->db->order_by($field["eng_date"].' ASC');
				$data = $this->db->get($table['table_name'])->result_array();
				foreach($data as $date){
					if($date[$field['eng_date']]){
	...
	public function update_date()
	{
		...
		foreach ($tables['fields'] as $field) {
			...
			$this->db->limit(500,11500);
			$data = $this->db->get('sales_vehicle_process')->result_array();
			foreach($data as $date){
				if($date[$field['eng_date']]){
					$np_date = get_nepali_date($date[$field['eng_date']],'true');
					$np_dates = explode('-', $np_date);
					...
					// echo $nepali_date;
					$this->db->where('id',$date['id']);
					$this->db->update('sales_vehicle_process',$nepali_date);
