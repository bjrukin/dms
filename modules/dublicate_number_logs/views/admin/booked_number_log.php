	...
	<section class="content">
		...
		<div class="row">
			<div class="col-xs-12 connectedSortable">
				...
				<div id='jqxGridDublicate_booked_number_logToolbar' class='grid-toolbar'>
					
					<button type="button" class="btn btn-danger btn-flat btn-xs" id="jqxGridDublicate_booked_number_logFilterClear"><?php echo lang('general_clear'); ?></button>
				</div>
				<div id="jqxGridDublicate_booked_number_log"></div>
			</div><!-- /.col -->
		</div>
		<!-- /.row -->
	</section><!-- /.content -->
...
$(function(){
	...
	var dublicate_number_logsDataSource =
	{
		...
		url: '<?php echo site_url("admin/dublicate_number_logs/bookedjson"); ?>',
	    ...
	    filter: function () {
	    	$("#jqxGridDublicate_booked_number_log").jqxGrid('updatebounddata', 'filter');
	    },
	    // update the grid and send a request to the server.
	    sort: function () {
	    	$("#jqxGridDublicate_booked_number_log").jqxGrid('updatebounddata', 'sort');
	    },
	    processdata: function(data) {
	    }
	};
	
	$("#jqxGridDublicate_booked_number_log").jqxGrid({
		theme: theme,
		width: '100%',
		height: gridHeight ,
		...
		rendertoolbar: function (toolbar) {
			...
			container.append($('#jqxGridDublicate_booked_number_logToolbar').html());
	...
	$("[data-toggle='offcanvas']").click(function(e) {
	    e.preventDefault();
	    setTimeout(function() {$("#jqxGridDublicate_booked_number_log").jqxGrid('refresh');}, 500);
	});

	$(document).on('click','#jqxGridDublicate_booked_number_logFilterClear', function () { 
		$('#jqxGridDublicate_booked_number_log').jqxGrid('clearfilters');
	});

	$(document).on('click','#jqxGridDublicate_booked_number_logInsert', function () { 
