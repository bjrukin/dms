...
<script type="text/javascript">
    ...
    function generateReport() {
        ...
        $.ajax({
            type: "POST",
            type: "POST",
            url: '<?php echo site_url("admin/spareparts_report/dealer_sales_report"); ?>',
            data: data,
                ...
                if (result.success && result.total >= 1) {                   
                    ...
                    var table =  $('#data-table').DataTable({
                        ...
                        "footerCallback": function ( row, data, start, end, display ) {
                            ...
                            total_sales = api.column( 8 ).data().reduce( function (a, b) {
                                return intVal(a) + intVal(b);
                            },0 );
                            total_taxable = api.column( 5 ).data().reduce( function (a, b) {
                                return intVal(a) + intVal(b);
                            },0 );

                            // total_quantity_final = api.column( 3, { page: 'current'} ).data().reduce( function (a, b) {
                            //     return intVal(a) + intVal(b);
                            // }, 0 );
                            // total_quantity_final = parseFloat(total_quantity_final);
                            total_quantity = parseFloat(total_quantity);
                            total = parseFloat(total_quantity);
                            total_taxable = parseFloat(total_taxable);
                            $('#totalquantity').html("Total : "+total_quantity);               
                            $('#totalsales').html("Total : "+total_sales);               
                            $('#total_taxable').html("Total : "+total_taxable); 
                        }
                    });
                    $('#report-table').show();
                } else {
    ...
    function buildHtmlTable(records,tableid,footer) {
        ...
        var footerTfoot$ = $('<tfoot><tr><th colspan="4" id = "totalquantity" style="text-align:right;"></th><th colspan="2" id = "total_taxable" style="text-align:right;"></th><th colspan="3" id = "totalsales" style="text-align:right;"></th></tr></tfoot>');
