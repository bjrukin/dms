  <?php echo form_open('', array('id' =>'form-counter_sale', 'onsubmit' => 'return false', 'class' => '')); ?>
  <div class="row">
  	<div class="col-md-6">
  		<div class="row">
  		...
  		<div class="row form-group"> 
  			...
  			<div class="col-md-2"> Counter No.</div>
  			<div class="col-md-5 ">
  				<div class="input-group">
  					<input id='counter_sales_id' class="form-control input-sm" name="counter_sales_id" placeholder="Empty for new." >
  					<div class="input-group-addon"><i class="fa fa-search"></i></div>
  				</div>
  			</div>
  			<div class="col-md-1"><button type="button" class="btn btn-flat btn-default" onclick="new_counter_sales()">New</button></div>

  		</div>
  		<div class="row form-group">
  			<div class="col-md-2">Invoice No.</div>
  			<div class="col-md-10 form-inline">
  				<input type="text" name="invoice_no-prefix" class="form-control input-sm">

  				<div class="input-group">
  					<input type="text" name="invoice_no" class="form-control input-sm" value="<?php echo $bill_id?>">
  					<div class="input-group-addon"><i class="fa fa-search" onclick="findCounterInvoice()"></i></div>
  				</div>
  			</div>
  		</div>
	  <!-- <div class="row">
...
<div class="col-md-6">
	<fieldset>
		...
		<div class="row">
			...
			<div class="col-md-4"><input type="text" name="vehicle_id" class="form-control input-sm" ></div>
			<div class="col-md-2">Variant</div>
			<div class="col-md-4"><input type="text" name="variant_id" class="form-control input-sm" ></div>
		</div>
		<!-- <div class="row">
...
Part_form_table_counter = $("#materialCounterJqxgrid").jqxGrid(
{
  ...
  function get_part_detail_counter(){
  	var part_detail = $('#new_part_id_counter').jqxComboBox('getSelectedItem')['originalItem']; 
	$('#new_part_code_counter').html(part_detail.part_code);
	$('#new_part_price_counter').val(part_detail.price);
	$('#new_part_name_counter').val(part_detail.name);
	$('#new_min_price_counter').val(part_detail.price);
  ...
  function new_counter_sales() {
  	...
  	$('#form-counter_sale input[name=issue_date]').val('');
  	// $('#counter_sale-credit_account').jqxComboBox('selectItem',invoice.credit_account);
  	// invoice_no
  	$('#form-counter_sale input[name=vehicle_no]').val('');
  	$('#form-counter_sale input[name=engine_no]').val('');
  	$('#form-counter_sale input[name=chassis_no]').val('');
  	$('#form-counter_sale input[name=vehicle_id]').val('');
  	$('#form-counter_sale input[name=variant_id]').val('');
  	$('#form-counter_sale input[name=color_id]').val('');

  	Part_form_table_counter.jqxGrid('updatebounddata');
  	$('#counter_summary-total_for_parts').val('');
  	calculate_counter_summary_percent();
  	$('#save_counter_sales').show();
  	$.post('<?php echo site_url('job_cards/counter/get_billing_number/json'); ?>',function(invoice){
  		$('#form-counter_sale input[name=invoice_no]').val(invoice);
  	}, 'json' );
  ...
  function findCounterInvoice() {
  	...
  	$.post('<?php echo site_url('job_cards/counter/findCounterInvoice'); ?>',{
  		...
  		$('#form-counter_sale input[name=engine_no]').val(invoice.engine_no);
  		$('#form-counter_sale input[name=chassis_no]').val(invoice.chasis_no);
  		$('#form-counter_sale input[name=vehicle_id]').val(invoice.vehicle_id);
  		$('#form-counter_sale input[name=variant_id]').val(invoice.variant_id);

	  // $('#form-counter_summary input[name=total_for_parts]').val(invoice.total_parts);

	  $.each(result.row, function(i,v){
	  	var datarow = {
	  		'id'          : v.ser_parts_id,
	  		'part_id'     : v.part_id,
	  ...
	  $('#counter_sales_id').val(invoice.counter_sales_id);
