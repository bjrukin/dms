						...
						<fieldset>
							...
							<div class="row form-group">
								...
								<div class="col-md-4">
									...
									<div id="jobcard_status-jobcardno_display" class="form-control" readonly></div>
								</div>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="btn-group btn-sm pull-right">
								<button id="job_status_exit" class="btn btn-default btn-flat">Exit</button>
							</div>
						</div>
					</div>
				</div>
		...
		$('.job-status').on('click',function(){
			...
			$.post('<?php echo site_url("job_cards/get_job_status")?>',{jobcard_group: jobno},function(result){
				if(result.success) {
					...
					$('#jobcard_status-jobcardno_display').text("JC-"+ (result.details.jobcard_serial).pad(5));
