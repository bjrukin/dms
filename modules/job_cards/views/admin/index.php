	...
	<div class="form_fields_area">
		...
		<div class="col-md-12">
				...
				<div class="col-md-6">
					...
					<div class="row form-group">
						...
						<div class="row form-group">
							...
							<div class="col-md-9">
								<label><div id="service_history"></div></label>
							</div>
						</div>
					</div>	
					<div class="col-md-6">
					...
					<div class="row form-group">
						...
						<div class="col-md-3 ">
							<label for='mechanicsid'><?php echo lang('technician_group_head')?></label>
						</div>
						<div class="col-md-9 ">
							<div id='mechanicsid' class='form-control input-sm' name='mechanics_id'></div>
						</div>
					</div>
					<div class="row form-group">
					...
					<div class="row form-group" id="tire_and_battery" hidden>

							<div class="col-md-3 ">
								<label for='tire_make'>Tire Make</label>
							</div>
							<div class="col-md-4 ">
								<div id="tire_make" name="tire_make" class="form-control"></div>
							</div>
							<div class="col-md-2 ">
								<label for='battery_no'>Battery No.</label>
							</div>
							<div class="col-md-3 ">
								<input id='battery_no' class='form-control input-sm' name='battery_no'>
							</div>
			...
			<div class="row">
				...
				<div class="col-md-2"> <div id='party_name'  name='party_id'></div> </div>
				<div class="col-md-2"> <input id="party_contact_number" class="text_input" readonly></div> 

				<div class="col-md-1"><label><?php echo ("Reciever Name"); ?></label></div>
				<div class="col-md-2"><input type="text" name="reciever_name" class="form-control" id="job_cards-reciever_name"></div>
...
<script language="javascript" type="text/javascript">
	...
	$(function(){
		...
		$('#tire_make').jqxDateTimeInput({ width: '50%', height: '18px', formatString: "yyyy-MM-dd" });
		...
		$('#job_cards-issue_date ').jqxDateTimeInput('setMaxDate', end);
	...
	$("#service_type").on('select', function (event) {
		if (event.args) {
			...
			if (item.type == 'pdi') 
			{
				...
				$('#tire_and_battery').show();

				$.post('<?php echo site_url('admin/job_cards/getPDI_jobs')?>',function(data){
		...
		if (event.args) {
			...
			if (item) {
				var variant_dataSource = {
					url : '<?php echo site_url("admin/job_cards/get_variants_combo_json_service"); ?>',
					data: {vehicle_id: item.value},
					datatype: 'json',
					datafields: [
					{ name: 'vehicle_id', type: 'number' },
		...
		if (event.args) {
			...
			if (item) {
				var colors_dataSource = {
					url : '<?php echo site_url("admin/job_cards/get_colors_combo_json_service"); ?>',
	...
	$("#service_advisor_id").jqxComboBox({
		...
		$("#mechanicsid, #combo_mechanics, #add_outside_work-mechanic_id").jqxComboBox({
			theme: theme,
			width: 195,
			height: 25,
			selectionMode: 'dropDownList',
		...
		$('#mechanicsid').on('select',function(event){
			if (event.args) {
				var item = event.args.item;
	...
	var job_cardsDataSource =
	{
		...
		{ name: 'tire_make', type: 'date' },
		{ name: 'battery_no', type: 'string' },
		{ name: 'material_issued_date', type: 'date' },
		...
		$("#jqxGridJob_card").jqxGrid({
			...
			cellsrenderer: function (index,b,c,d,e,rows) {
				// console.log()
				var p = ' | ';
				var edit = m = e = a = i = x = o = floor = r = returnvalues = viewJC = '';
				...
				x = ' <a href="javascript:void(0)" onclick="sale_return(' + index + '); return false;" title="<?php echo lang('sale_return')?>"><i class="fa fa-reply"></i></a>' +p;
				...
				<?php if(is_service_advisor() || is_floor_supervisor()):?>
					console.log(rows.service_adviser_id);
					if(  (rows.floor_supervisor_id == <?php echo $this->session->userdata('id')?>)){
						returnvalues += edit + a;
					}else{
						returnvalues += edit + a;
					}
				<?php endif; ?>
				<?php if(is_material_issuer() || is_workshop_manager()):?>
				returnvalues += m ;
				<?php endif; ?>
				<?php if(is_floor_supervisor()):?>
		...
		{ text: '<?php echo 'Material Issued Status'; ?>',datafield: 'material_issued_date', columntype:'checkbox', width: 75,filterable: true,renderer: gridColumnsRenderer, cellclassname: cellclassrenderer, filterable: false },
		{ text: 'Billed Status', datafield: 'issue_date', sortable:false, width: 75,filterable:false, align: 'left' , cellsalign: 'left', cellclassname: 'grid-column-left', exportable: false, cellclassname: cellclassrenderer,renderer: gridColumnsRenderer, cellsrenderer: function (index,b,c,d,e,rows) {
				if(rows.issue_date){
					return '<div style="position: absolute; top: 50%; left: 50%; margin-top: -7px; margin-left: -10px; cursor: auto;" id="jqxWidget58a3b760" tabindex="0" class="jqx-widget jqx-widget-bootstrap jqx-checkbox jqx-checkbox-bootstrap"><div class="jqx-checkbox-default jqx-checkbox-default-bootstrap jqx-fill-state-normal jqx-fill-state-normal-bootstrap jqx-rc-all jqx-rc-all-bootstrap"><div style="width: 13px; height: 13px;"><span style="width: 13px; height: 13px;" class="jqx-checkbox-check-checked jqx-checkbox-check-checked-bootstrap"></span></div></div><div style="clear: both;"></div></div>';
				}else{
					return '<div style="position: absolute; top: 50%; left: 50%; margin-top: -7px; margin-left: -10px; cursor: auto;" id="jqxWidgetd07a1e82" tabindex="0" class="jqx-widget jqx-widget-bootstrap jqx-checkbox jqx-checkbox-bootstrap"><div class="jqx-checkbox-default jqx-checkbox-default-bootstrap jqx-fill-state-normal jqx-fill-state-normal-bootstrap jqx-rc-all jqx-rc-all-bootstrap"><div style="width: 13px; height: 13px;"><span style="width: 13px; height: 13px;"></span></div></div><div style="clear: both;"></div></div>';
				}
			}
		},
		{ 
		...
		{ text: 'Invoice No',datafield: 'invoice_no',width: 150,filterable: true,renderer: gridColumnsRenderer,  cellclassname: cellclassrenderer,},
		{ text: 'Customer Name',datafield: 'customer_name',width: 150,filterable: true,renderer: gridColumnsRenderer,  cellclassname: cellclassrenderer,},
...
$(document).on('click','#jqxGridJob_cardInsert', function () { 
	...
	$("#jqxPopupWindow_AddOutsideWork").jqxWindow({ 
		...
		$('#form-job_cards').jqxValidator({
			...
			{ input: "#battery_no", message: 'Required', action: 'blur',
			rule: function(input) {
				var service_type = $('#service_type').jqxComboBox('getSelectedItem');
				...
				if (service_type.originalItem.type == 'pdi' ) {
					val = $('#battery_no').val();
					return (val == '' || val == null || val == 0) ? false: true;
				}
				return true;
			...
			{ input: "#tire_make", message: 'Required', action: 'blur',
			rule: function(input) {
				var service_type = $('#service_type').jqxComboBox('getSelectedItem');
				// console.log(service_type);
				if( ! service_type) {
...
$("#jqxJob_cardSubmitButton").on('click', function () {
		...
		Job_form_table = $("#jqxgrid").jqxGrid(
		{
			...
			{ text: '<?php echo lang("delete_a_row") ?>', datafield: 'Delete', width: '5%', columntype: 'button', cellsrenderer: function() { return "Delete"; }, buttonclick: function(row,detail) {
				...
				var row  = Job_form_table.jqxGrid('getrows');
				if(row.length > 1){
					...
					alert("Please add another Job to delete this Job");
...
function editJob_cardRecord(index){
	...
	if (row) {
		...
		$.post('<?php echo site_url("admin/job_cards/get_jobCard_details"); ?>',{ jobcard_group: row.jobcard_group, vehicle_no: row.vehicle_no}, function(result){
			...
			// $('#mechanics_id').jqxComboBox('val',row.mechanics_id);
			...
			// $('#mechanicsid').val(details.mechanicsid);
			$('#mechanicsid').jqxComboBox('val',details.mechanics_id);
			$('#cleaner_id').val(details.cleaner_id);
			// $('#mechanic_list').val(details.mechanic_list);
			$('#mechanic_list').jqxComboBox('val',details.mechanic_list);
			...
			if(details.service_type==8){
				$('#tire_make').val(details.tire_make);
				$('#battery_no').val(details.battery_no);
			}
...
function reset_form_job_cards(){
	...
	$('#mechanicsid').jqxComboBox('clearSelection'); 
...
function sale_return(index) {
	var row =  $("#jqxGridJob_card").jqxGrid('getrowdata', index);
	window.open(base_url+"job_cards/billing/sale_return?jobcard_group="+row.jobcard_group,'_black');
}
...
function outside_work(index) {
	...
	$("#combo_job_id").jqxComboBox({
	 ...
	 $('#job_group').val(row.jobcard_group);
	 ...
	 var outside_workDataSource =
	 {
	 	...
	 	{ name: 'splr_invoice_date', type: 'string' },
...
<script type="text/javascript">
	$('#chassis_no,#engine_no,#vehicle_register_no').on('change',function(){
		var data = $(this).val();
		$.post("<?php echo site_url('job_cards/get_kms')?>",{data: data},function(result){
			console.log(result);
			if(result) {
				if(result.closed_status == 0){
					alert('Same Vehicle job exist with job number '+result.jobcard_serial);
				}
				...
				$('#party_contact_number').val(result.mobile);
				...
				$('#service_history').html('<a href="<?php echo site_url('admin/job_cards/show_service_history') ?>?chassis_no='+result.chassis_no+'" target="_blank">Show Vehicle History</a>')
