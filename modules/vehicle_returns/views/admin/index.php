...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1><?php echo lang('vehicle_returns'); ?></h1>
		<ol class="breadcrumb">
	        <li><a href="#">Home</a></li>
	        <li class="active"><?php echo lang('vehicle_returns'); ?></li>
      </ol>
	</section>
	<!-- Main content -->
	<section class="content">
		...
		<div class="row">
			<div class="col-xs-12 connectedSortable">
				...
				<div id='jqxGridVehicle_returnToolbar' class='grid-toolbar'>
					<button type="button" class="btn btn-primary btn-flat btn-xs" id="jqxGridVehicle_returnInsert"><?php echo lang('general_create'); ?></button>
					<button type="button" class="btn btn-danger btn-flat btn-xs" id="jqxGridVehicle_returnFilterClear"><?php echo lang('general_clear'); ?></button>
				</div>
				<div id="jqxGridVehicle_return"></div>
			</div><!-- /.col -->
		</div>
		<!-- /.row -->
	</section><!-- /.content -->
...
<div id="jqxPopupWindowVehicle_return">
   <div class='jqxExpander-custom-div'>
        <span class='popup_title' id="window_poptup_title"></span>
    </div>
    <div class="form_fields_area">
        <?php echo form_open('', array('id' =>'form-vehicle_returns', 'onsubmit' => 'return false')); ?>
        	<input type = "hidden" name = "id" id = "vehicle_returns_id"/>
            <table class="form-table">
                ...
                <tr>
                    <th colspan="2">
                        <button type="button" class="btn btn-success btn-xs btn-flat" id="jqxVehicle_returnSubmitButton"><?php echo lang('general_save'); ?></button>
                        <button type="button" class="btn btn-default btn-xs btn-flat" id="jqxVehicle_returnCancelButton"><?php echo lang('general_cancel'); ?></button>
                    </th>
                </tr>
...
$(function(){
	...
	var vehicle_returnsDataSource =
	{
		...
		url: '<?php echo site_url("admin/vehicle_returns/json"); ?>',
        ...
        beforeprocessing: function (data) {
        	vehicle_returnsDataSource.totalrecords = data.total;
        },
	    // update the grid and send a request to the server.
	    filter: function () {
	    	$("#jqxGridVehicle_return").jqxGrid('updatebounddata', 'filter');
	    },
	    // update the grid and send a request to the server.
	    sort: function () {
	    	$("#jqxGridVehicle_return").jqxGrid('updatebounddata', 'sort');
	    },
	    processdata: function(data) {
	    }
	};
	
	$("#jqxGridVehicle_return").jqxGrid({
		theme: theme,
		width: '100%',
		height: gridHeight,
		source: vehicle_returnsDataSource,
		altrows: true,
		pageable: true,
		sortable: true,
		rowsheight: 30,
		...
		rendertoolbar: function (toolbar) {
			...
			container.append($('#jqxGridVehicle_returnToolbar').html());
		...
		columns: [
			{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
			{
				...
				cellsrenderer: function (index) {
					var e = '<a href="javascript:void(0)" onclick="editVehicle_returnRecord(' + index + '); return false;" title="Edit"><i class="fa fa-edit"></i></a>';
	...
	$("[data-toggle='offcanvas']").click(function(e) {
	    e.preventDefault();
	    setTimeout(function() {$("#jqxGridVehicle_return").jqxGrid('refresh');}, 500);
	});

	$(document).on('click','#jqxGridVehicle_returnFilterClear', function () { 
		$('#jqxGridVehicle_return').jqxGrid('clearfilters');
	});

	$(document).on('click','#jqxGridVehicle_returnInsert', function () { 
		openPopupWindow('jqxPopupWindowVehicle_return', '<?php echo lang("general_add")  . "&nbsp;" .  $header; ?>');
    });

	// initialize the popup window
    $("#jqxPopupWindowVehicle_return").jqxWindow({ 
        theme: theme,
        width: '75%',
        maxWidth: '75%',
        height: '75%',  
    ...
    $("#jqxPopupWindowVehicle_return").on('close', function () {
        reset_form_vehicle_returns();
    });

    $("#jqxVehicle_returnCancelButton").on('click', function () {
        reset_form_vehicle_returns();
        $('#jqxPopupWindowVehicle_return').jqxWindow('close');
    });

    /*$('#form-vehicle_returns').jqxValidator({
        hintType: 'label',
    ...
    $("#jqxVehicle_returnSubmitButton").on('click', function () {
        saveVehicle_returnRecord();
        /*
        ...
        $('#form-vehicle_returns').jqxValidator('validate', validationResult);
        */
    });
});

function editVehicle_returnRecord(index){
    var row =  $("#jqxGridVehicle_return").jqxGrid('getrowdata', index);
  	if (row) {
  		$('#vehicle_returns_id').val(row.id);
        ...
        openPopupWindow('jqxPopupWindowVehicle_return', '<?php echo lang("general_edit")  . "&nbsp;" .  $header; ?>');
    }
}

function saveVehicle_returnRecord(){
    var data = $("#form-vehicle_returns").serialize();
	
	$('#jqxPopupWindowVehicle_return').block({ 
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo site_url("admin/vehicle_returns/save"); ?>',
        data: data,
        success: function (result) {
            var result = eval('('+result+')');
            if (result.success) {
                reset_form_vehicle_returns();
                $('#jqxGridVehicle_return').jqxGrid('updatebounddata');
                $('#jqxPopupWindowVehicle_return').jqxWindow('close');
            }
            $('#jqxPopupWindowVehicle_return').unblock();
        }
    });
}

function reset_form_vehicle_returns(){
	$('#vehicle_returns_id').val('');
    $('#form-vehicle_returns')[0].reset();
}
</script>