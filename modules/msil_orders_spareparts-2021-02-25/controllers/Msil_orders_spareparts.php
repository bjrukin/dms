...
class Msil_orders_spareparts extends Project_Controller
{
    ...
    public function grouped_dispatch()
    {
        ...
        foreach($rows as $key =>$value)
        {
            ...
            $where['in_stock'] = 0;
            $this->msil_order_model->_table = 'view_binned_msil_order';
    ...
    public function msil_dispatch_json()
    {
        ...
        foreach ($rows as $key => $value) {
            ...
            if($value->binning_status == 1)
            {
                $rows[$key]->binning_status= 'Yes';
            }else{
                $rows[$key]->binning_status= 'No';
            }
        }
   ...
   //  }
    ...
    public function print_binning($invoice_no)
    {
        ...
        //$this->msil_order_model->_table = "ser_workshop_users";
        $this->msil_order_model->_table = "dms_msil_scanned_order";
        $rows = $this->msil_order_model->findAll($where);
        foreach ($rows as $key => $value) {
            $this->db->where('id',$value->scanner_name_id);
            $person  = $this->db->get('ser_workshop_users')->row();
            $rows[$key]->scanner_name = $person->first_name;

            $this->db->where('id',$value->scanner_device_id);
            $device  = $this->db->get('mst_scan_device')->row();
            $rows[$key]->device_name = $device->device_name;
        }
        $data['rows'] = $rows ;
        $data['header'] = lang('msil_orders');
    ...
    public function get_scanned_values()
    {
        ...
        $location = $this->input->post('location');
        $scanner_name_id = $this->input->post('scanner_name');
        $scanner_device_id = $this->input->post('scanner_device');
        $part_code = $this->input->post('code');
        $invoice_no = $this->input->post('invoice');
    //check quantity
        $this->db->where('part_code',$part_code);
        $this->db->where('invoice_no',$invoice_no);
        $this->db->select('sum(box_quantity * quantity) As qnty');
        $this->db->group_by('part_code');
        $this->msil_order_model->_table = 'dms_msil_scanned_order';
        $scanned_quantity = $this->msil_order_model->find(NULL,NULL);
        if($scanned_quantity)
        {

            $total_binned_qnty =  $scanned_quantity->qnty + 1;
        }else{
            $total_binned_qnty = 1;
        }
        $this->msil_order_model->_table = "view_msil_received_order";
        $this->db->where('binning_status IS NULL');
        $this->db->select('sum(quantity) as invoice_qnty,part_name');
        $this->db->group_by('part_code,part_name');
        $rows = $this->msil_order_model->find(array('invoice_no'=>$invoice_no,'part_code'=>$part_code),NULL);
       //  echo $this->db->last_query();
      // echo "<pre>";
      // print_r($scanned_quantity);
      // print_r($rows->invoice_qnty);
      // die();
       // print_r($rows);die();
        $data = array();
        $success = false;
        ...
        { 
            ...
            if($rows->invoice_qnty >= $total_binned_qnty ){
                ...
                $scanned_orders = $this->msil_order_model->find();
                     // echo $this->db->last_query();
                     //     print_r($scanned_orders);die();
                $data['part_name'] = $rows->part_name;
                $data['scanner_name_id'] = $scanner_name_id;
                $data['scanner_device_id'] = $scanner_device_id;
                $data['box_quantity'] = 1;
                $data['quantity'] = 1;
                ...
                if($scanned_orders)
                {
                    ...
                    $data['box_quantity'] =  $data['box_quantity']+$scanned_orders->box_quantity;
                ...
                $success = false;
                $msg = "Quantity  ".$total_binned_qnty." is unavailable for  partcode ".$part_code;
            }
        }else{
            $success = false;
    ...
    public  function update_scanned_quantity()
    {
        ...
        if($scanned_quantity)
        {
            ...
            $total_binned_qnty = ($scanned_quantity->qnty)+ $data['quantity'];
        }else{
          $total_binned_qnty =  $data['quantity'];
        }

        $this->msil_order_model->_table = "view_msil_received_order";
        $fields = array();
        $fields[] = 'sum(quantity) as total';
        $rows = $this->msil_order_model->find(array('invoice_no'=>$invoice_no,'part_code'=>$part_code),$fields);
          // echo "<pre>";
          // print_r($scanned_quantity);
          // print_r($rows->total);
          // print_r($total_binned_qnty);
          // die();
// echo $total_binned_qnty;
// echo $rows->total;
        if($rows->total >= $total_binned_qnty)
        {
            ...
            $msg = $rows->total.' Quantity only available for partcode '.'"' .$part_code.'"';           
            $success = FALSE;    
        }
   ...
   //          }
    ...
    public function confirm_binning()
    {
        ...
        $where['in_stock'] = 0;
        $this->msil_order_model->_table = 'view_binned_msil_order';
        $dispatched_rows = $this->msil_order_model->findAll($where);
          // echo $this->db->last_query();die();
        $stock_rows = array();
        $this->msil_order_model->_table = 'spareparts_msil_order';
        $stock_new_location = array();
        foreach ($dispatched_rows as $key => $value) 
        {
            if(!isset($stock_new_location[$value->mst_part_id])){
              $stock_new_location[$value->mst_part_id] = array();
            }
            $stock_rows = $this->sparepart_stock_model->find(array('sparepart_id'=>$value->mst_part_id));
            if(($stock_rows->quantity ==0) && ($value->location!=$value->binning_location)){
                  // $stock['location'] = $value->binning_location;
                if(!in_array($value->binning_location,$stock_new_location[$value->mst_part_id])){
                    $stock_new_location[$value->mst_part_id][] = $value->binning_location;
                }

            }else if(($stock_rows->quantity >=0) &&  ($value->location != $value->binning_location))
            {
                  // $new_location1 = explode('/', $value->location);
                  // $new_location2 = explode('/', $value->binning_location);
                  // $temp_location = array_unique(array_merge($new_location1,$new_location2));
                  // $temp_location_=array_filter($temp_location, function($value) { return !is_null($value) && $value !== ''; });
                  // $location = implode('/',$temp_location_);
                  // $stock['location'] = $location; 
                if(!in_array($value->location,$stock_new_location[$value->mst_part_id])){
                    $stock_new_location[$value->mst_part_id][] = $value->location;
                }
                if(!in_array($value->binning_location,$stock_new_location[$value->mst_part_id])){
                    $stock_new_location[$value->mst_part_id][] = $value->binning_location;
                }
               ...
               // $stock['location'] = $value->location;
                if(!in_array($value->binning_location,$stock_new_location[$value->mst_part_id])){
                    $stock_new_location[$value->mst_part_id][] = $value->binning_location;
                }
            }
            if($value->binning_quantity == $value->quantity)
            {
              $data['in_stock'] = 1;
            ...
            // echo "<pre>";
            // print_r($data);die();
            $total_binning_qnty =  $value->binning_quantity * $value->box_quantity;
            if($stock_rows)
            {
              $stock['id'] = $stock_rows->id;
              $stock['location'] = implode('/',$stock_new_location[$value->mst_part_id]);
              $stock['quantity'] = $stock_rows->quantity + $total_binning_qnty;
              $this->sparepart_stock_model->update($stock['id'],$stock);
              ...
              $success = $this->msil_order_model->update($data['id'],$data);
                ...
                $stock_data = array(
                    'sparepart_id' => $value->mst_part_id,
                    'quantity' => $total_binning_qnty,
                    'location' => implode('/',$stock_new_location[$value->mst_part_id]),
                );
    ...
    public function get_scan_person_combo_json()
    {
    ...
    public function get_scan_device_combo_json()
    {

        $rows = $this->scan_device_model->findAll();
        array_unshift($rows, array('id' => '0', 'device_name' => 'Select'));
