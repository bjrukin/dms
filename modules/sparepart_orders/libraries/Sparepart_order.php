...
class Sparepart_order{
	...
	public function generate_proforma_invoice($order_no,$pi_id,$export_type,$dealer_details,$dealer_id)
	{
		...
		$this->CI->sparepart_order_model->_table = 'view_spareparts_order';

		/*$result = $this->CI->sparepart_order_model->findAll(array('order_no'=>$order_no,'dealer_id'=>$dealer_id));	
		$fields = 'dealer_name,created_at,pi_generated_date_time,order_type,district_name,address_1,dispatch_mode';
		...
		$fields = 'id,dealer_name,created_at,pi_generated_date_time,order_type,district_name,address_1,dispatch_mode,pi_number,part_code,name,order_quantity,dealer_price';
		$this->CI->db->group_by($fields);
		$result = $this->CI->sparepart_order_model->findAll(array('order_no'=>$order_no,'dealer_id'=>$dealer_id),$fields);	
		$fields = 'dealer_name,created_at,pi_generated_date_time,order_type,district_name,address_1,dispatch_mode';
		$this->CI->db->group_by($fields);
		$header_info = $this->CI->sparepart_order_model->find(array('order_no'=>$order_no,'dealer_id'=>$dealer_id),$fields);
		...
		if(!empty($result))
		{
			if(!$result[0]->pi_number)
			{
				...
				foreach ($result as $value) {
					...
					$sparepart = $this->CI->sparepart_model->find(array('id'=>$value->sparepart_id));
					$data['pi_cost'] = $sparepart->price;
				...
				$pi_id = $this->CI->sparepart_order_model->find($where_pi,'max(proforma_invoice_id) as proforma_invoice_id');
			}
			...
			if($export_type == 1)
			{
				...
				->setCellValue('B8', 'Dispatch Mode')
				->setCellValue('C8', $header_info->dispatch_mode)
...
*/		return $success;	
	...
	public function get_new_invoice_no($stockyard_id)
	{
		$fiscal_year = get_current_fiscal_year();
		$this->CI->load->model('dispatch_spareparts/dispatch_sparepart_model');

		$where_bill_start_date["dispatched_date >= '" .$fiscal_year['2'] . "'"] = NULL;
		$where_bill_start_date["stockyard_id"] = $stockyard_id;
		$this->CI->db->group_by('dispatched_date_nepali, bill_no');
		$this->CI->db->order_by('bill_no','desc');
		$result = $this->CI->dispatch_sparepart_model->find($where_bill_start_date,'bill_no,dispatched_date_nepali');

		$this->CI->db->order_by('invoice_no','DESC');
		$where_countersales_bill = array(
			'stockyard_id' => $stockyard_id,
			"issue_date >= '" . $fiscal_year['2'] . "'" => NULL,
			"invoice_no IS NOT " => NULL,

		);
		$this->CI->sparepart_stock_model->_table = 'spareparts_stockyard_countersales';
		$countersales_bill = $this->CI->sparepart_stock_model->find($where_countersales_bill);
		if($countersales_bill && $result->bill_no < $countersales_bill->invoice_no){
			$result->bill_no = $countersales_bill->invoice_no;
		}
	...
	public function check_bill($bill_no='',$stockyard_id, $fiscal_year)
	{
		$result = $this->CI->dispatch_sparepart_model->find(array('bill_no'=>$bill_no,"dispatched_date >='" . $fiscal_year . "'" => NULL, "stockyard_id" => $stockyard_id));
		$this->CI->sparepart_stock_model->_table = 'spareparts_stockyard_countersales';
		$result1 = $this->CI->dispatch_sparepart_model->find(array('invoice_no'=>$bill_no,"issue_date >='" . $fiscal_year . "'" => NULL, "stockyard_id" => $stockyard_id));
