...
class Sparepart_orders extends Project_Controller
{
	...
	public function pi_indexed_json()
	{
		...
		$group = "proforma_invoice_id,dealer_name,dealer_id,order_no,order_concat,order_type,pi_number,pi_generated_date_time,remarks,dispatch_mode,pick_count";
		$fields = "proforma_invoice_id,dealer_name,dealer_id,order_no,order_concat,order_type,pi_number,pi_generated_date_time,SUM(order_quantity) as order_qty,count(*) as total_line_qty, SUM(order_quantity * dealer_price) as total_amount,remarks,dispatch_mode,pick_count";
	...
	public function generate_picking_list()
	{
		...
		foreach ($rows as $key => $value) 
		{
			...
			{*/
				...
				$picklist[$key]['sparepart_id'] = $value->sparepart_id;
				...
				$print_picklist[$key]['part_code'] = $sparepart->part_code;
...
// exit;
			...
			{
				$this->db->order_by('id');
				$this->db->where('received_quantity < order_quantity');
				$order = $this->sparepart_order_model->find(array('dealer_id'=>$data['dealer_id'],'sparepart_id'=>$result->sparepart_id,'picklist' => 1));
// echo '<pre>';
// echo $this->db->last_query();
// 				var_dump($order);
// 				exit;
...
// exit;
		...
		public function billing_list()
		{
			...
			$fields = array('bill_concat','bill_no','dealer_name','dispatched_date','dealer_id');
		...
		public function picking_list_json()
		{
			...
			$fields = "order,dealer_name,pick_count,order_no,dealer_id,picklist_no,picklist_format,billed_status,proforma_invoice_id,pi_number";
		...
		{
			$this->db->order_by('id','asc');
			$data = $this->sparepart_order_model->find(array('order_quantity > received_quantity'=>NULL,'dealer_id'=>$value['dealer_id'],'sparepart_id'=>$value['sparepart_id']));
			return $data;
		}
