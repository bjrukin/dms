...
class Dealer_orders extends Project_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library('dealer_orders/dealer_order');
    }
...
// Display Page
    ...
    public function json()
    {
        ...
        foreach ($rows as $key => &$value) {
            ...
            if($value->order_status != 'Dispatched'){
                ...
                $value->stock_status = $stock_status;
                $value->in_stock_remarks = $in_stock_remarks;

                
            }else{
                $value->stock_status = NULL;
                $value->in_stock_remarks = NULL;
            }
        }
    ...
    public function retail_json()
    {
        ...
        foreach ($rows as $key => &$value) {
            // print_r($value);
            if($value->order_status != 'Dispatched' && $value->credit_control_approval == 1){
                $stock_status = array();
                list ($stock_status, $in_stock_remarks) = $this->order_stock_availability_ktm($value->order_id, $value->vehicle_id, $value->variant_id, $value->color_id, $value->year, $value->is_ktm_dealer);
                $value->stock_status = $stock_status;
                $value->in_stock_remarks = $in_stock_remarks;
...
//load upload library
    ...
    public function get_order_status()
    {
        ...
        $fuel_count = $this->dealer_order->get_records('log_fuel_kms');
        ...
        foreach ($rows as $key => $value) {
            ...
            foreach ($fuel_count as $keys=>$fuel){
                if($value->vehicle_id==$fuel->vehicle_id){
                    $rows[$key]->fuel_quantity=$fuel->total_fuel_quantity;
                    break;
                }
            }
        }
...
// exit;
    ...
    private function order_stock_availability_ktm($order_id='', $vehicle_id='', $variant_id='', $color_id='', $mfg_year='', $is_ktm_dealer='')
    {
        $where = array(
            'vehicle_id' => $vehicle_id,
            'variant_id' => $variant_id,
        ...
        // $current_stock_count = "Select count(*) as count from msil_dispatch_records where (current_location = 'KATHMANDU' OR current_location = 'Satungal' OR current_location = 'Dhapakhel') AND current_status = 'Stock' AND vehicle_id = '".$vehicle_id."' AND variant_id = '".$variant_id."' AND color_id = '".$color_id."' AND year = '".$mfg_year."'";

        $query = "SELECT
                    COUNT(*) AS numrows 
                    FROM
                        msil_dispatch_records
                    WHERE
                        (
                            current_location = 'KATHMANDU'
                            OR current_location = 'Satungal'
                            OR current_location = 'Dhapakhel'
                        )
                    AND current_status = 'Stock'
        ...
        $current_stock_count = $this->db->query($query)->row_array();
...
// exit;
        ...
        // echo '<br>';
        // exit;
        if($current_stock_count['numrows'] > $order_count){
            return array(0=>'In Stock',1=>1);
        }
        ...
        if($current_stock_count['numrows'] + $transit_not - $order_count > 0){
            return array(0=>'In Transit',1=>2);
        }
