...
class Sparepart_orders extends Project_Controller
{
	...
	public function back_log()
	{
		...
		$where =array();
		// $this->sparepart_order_model->_table = 'view_spareparts_order';
		$fields = 'dealer_name, part_code, part_name as name, backorder AS total_backorder, pi_number, order_no_concat as order_no,id, order_type, dispatch_mode, dealer_price, location, current_stock';
		...
		if(is_sparepart_dealer() || is_workshop_manager() || is_aro())
		{
			$dealer_id = $this->session->userdata('employee')['dealer_id']; 
			$where = "(dealer_id = ".$dealer_id.")";
		}
		...
		foreach ($rows as $key => $value) {
			$rows[$key]->total_amount = $value->total_backorder * $value->dealer_price;
			if($value->current_stock < 0){
				$rows[$key]->current_stock = 0;
			}
			if($value->total_backorder > $value->current_stock){
				// if($value->current_stock && $value->current_stock > 0){
					$rows[$key]->min_bill_quantity = $value->current_stock;
					$rows[$key]->min_bill_amount = $value->current_stock * $value->dealer_price;
				// }else{
					// $rows[$key]->min_bill_quantity = 0;
					// $rows[$key]->min_bill_amount = 0;
				// }
			}else{
				$rows[$key]->min_bill_quantity = $value->total_backorder;
				$rows[$key]->min_bill_amount = $value->total_backorder * $value->dealer_price;
			}
		}
    ...
    public function dealer_order_import()
    {
		...
		foreach ($raw_data as $key => $value) {
			if($value['part_code']){
				if(is_numeric($value['order_quantity']))
				{
					$sparepart_id = $this->sparepart_model->find(array('part_code'=>strtoupper($value['part_code'])),array('id','moq'));
	   				...
	   				flashMsg('error', 'Please check the error in excel sheet and reupload');
	   				redirect($_SERVER['HTTP_REFERER']);
	   			} 
	...
	public function recent_dispatch_json()
	{
		...
		$fields = 'bill_no, dispatched_date, bill_concat, deleted_at, dealer_name';
		search_params();
		$this->db->group_by($fields);
		$this->db->select('count(bill_no)');
		$this->db->where($where);
		$count_array = $this->db->get($this->sparepart_order_model->_table)->result_array();
...
/*	public function get_barcode_values()
	{
		...
		public function get_barcode_values()
		{
			...
			if(is_group(SPAREPART_INCHARGE_GROUP) || is_group(703)){
	            ...
	            $where = array('stockyard_id' => $stockyard->stockyard_id);
	        }else{
	        	$success = FALSE;
				$message = 'Sorry you cannot bill';	
				echo json_encode(array('success'=>$success, 'msg'=>$message));
				exit;
	        }
			$result = $this->sparepart_stock_model->find(array('part_code'=>$data['part_code'], 'stockyard_id'=>$stockyard->stockyard_id ));
			...
			if($result)
			{
				...
				// print_r($this->db->last_query());echo ';<br>';
					...
					if($picklist)
					{
						$remaining = $this->check_remaining($data['dealer_id'],$result->sparepart_id);
						if($remaining){
							...
							$data['sparepart_id'] = $result->sparepart_id;
							$data['stockyard_id'] = $stockyard->stockyard_id;
							$success = $this->dispatch_list_model->insert($data);
						}else{
							$success = FALSE;
							$message = 'Alternate part is billed for this part';
						}
					}
					else
					{
		...
		public function check_remaining($dealer_id,$sparepart_id)
		{
			$this->db->where('order_quantity > received_quantity + cancle_quantity', null, false);
				$fields = 'SUM(order_quantity - received_quantity - cancle_quantity)';

				$remaining = 0;

				$order = $this->sparepart_order_model->find(array('dealer_id' => $dealer_id, 'sparepart_id'=>$sparepart_id, 'pi_generated' => 1, 'picklist' => 1), $fields);

				if($order){
					$remaining = $order->sum;
				}
				$part_ids[] = $sparepart_id;

				/* checking alternative orders */
				$part_detail = $this->sparepart_model->find(array('id'=>$sparepart_id));
				$alternative = array();
				...
				if (count($alternative)) {
					...
					if(count($where_alternative_ids)){
						$where_alternative_order['(' . implode(' OR ', $where_alternative_ids) . ')'] = NULL;
					}
					$where_alternative_order['dealer_id'] = $dealer_id;
					$where_alternative_order['pi_generated'] = 1;
					$where_alternative_order['picklist'] = 1;

					$this->db->where('order_quantity > received_quantity + cancle_quantity', null, false);
					$alternative_part_order = $this->sparepart_order_model->find($where_alternative_order,$fields);

					if($alternative_part_order){
						$remaining += $alternative_part_order->sum;
					}
				}

				$this->db->where('sparepart_id in ('. implode(',', $part_ids). ')');
				$where_ready_to_bill = array(
					'dealer_id' => $dealer_id,
					'is_billed' => 0,
				);
				$ready_to_bill_items = $this->dispatch_list_model->find($where_ready_to_bill,'SUM(dispatch_quantity)');

				/* end checking alternative parts*/
				if($remaining < 1 + $ready_to_bill_items->sum){
					return FALSE;
				}
				return TRUE;
		}
		...
		public function get_recent_dispatch_list()
		{
			...
			$strDate = substr($this->input->get('dispatched_date'),4,11);
			...
			$total = $this->dispatch_sparepart_model->find_count(array('bill_no'=>$bill_no,'grn_received_date'=>NULL, 'dispatched_date' => $dispatched_date));
		...
		public function get_received_order_list()
		{
			...
			$total = $this->dispatch_sparepart_model->find_count(array('bill_no'=>$bill_no,'grn_received_date <>'=>NULL));
			// print_r($this->db->last_query());
			// exit;
		...
		public function get_detailed_bill()
		{
			...
			$total=$this->sparepart_order_model->find_count(array('bill_no'=>$bill_no,'year_np'=>$date[0]));

			search_params();

			$rows=$this->sparepart_order_model->findAll(array('bill_no'=>$bill_no,'year_np'=>$date[0]));
		...
		public function picking_list_json()
		{
			...
			$fields = 'order_text,dealer_name,pick_count,order_no,dealer_id,picklist_no,picklist_format,billed_status,proforma_invoice_id,pi_number,picklist_group';
		...
		public function update_dispatch_quantity($bill = 1)
		{
			...
			if($dispatch_list_data->dispatch_quantity < $data['dispatch_quantity']){
				...
				$order = $this->sparepart_order_model->find(array('dealer_id' => $dispatch_list_data->dealer_id, 'sparepart_id'=>$dispatch_list_data->sparepart_id, 'pi_generated' => 1, 'picklist' => 1), $fields);
				// $order = $this->sparepart_order_model->findAll(array('dealer_id' => $dispatch_list_data->dealer_id, 'sparepart_id'=>$dispatch_list_data->sparepart_id, 'pi_generated' => 1, 'picklist' => 1), $fields);

				if($order){
					$remaining = $order->sum;
				}
				$part_ids[] = $dispatch_list_data->sparepart_id;
				...
				if (count($alternative)) {
					...
					$this->db->where('order_quantity > received_quantity + cancle_quantity', null, false);
					// $alternative_part_order = $this->sparepart_order_model->find_all($where_alternative_order,$fields);
					$alternative_part_order = $this->sparepart_order_model->find($where_alternative_order,$fields);
				...
				$this->db->where('sparepart_id in ('. implode(',', $part_ids). ')');
				$where_ready_to_bill = array(
					'dealer_id' => $dispatch_list_data->dealer_id,
					'is_billed' => 0,
				);
				$ready_to_bill_items = $this->dispatch_list_model->find($where_ready_to_bill,'SUM(dispatch_quantity)');
				...
				if($remaining < $data['dispatch_quantity'] + $ready_to_bill_items->sum - $dispatch_list_data->dispatch_quantity){
					$msg = 'Order Quantity Exceeded by '.($data['dispatch_quantity'] + $ready_to_bill_items->sum - $remaining - $dispatch_list_data->dispatch_quantity) ;
					$success = FALSE;
					echo json_encode(array('success'=>$success,'msg'=>$msg));
					exit;
				}
		...
		public function save_bill()
		{
			...
			if($this->input->post('bill_no')){
				...
				$result = $this->sparepart_order->check_bill($bill_no, $stockyard->stockyard_id, $fiscal_year[2]);
				if($result){
					$success = FALSE;
				...
				if($result){
					$result = $this->dispatch_sparepart_model->find(array('bill_no'=>$bill_no,"dispatched_date >='" . $fiscal_year[2] . "'" => NULL, "stockyard_id" => $stockyard->stockyard_id));
					$dispatched_date_nepali = $result->dispatched_date_nepali;
				}else{
					$dispatched_date_nepali = get_nepali_date(date('Y-m-d'),'nep');
				}

			}else{
				$where_bill_start_date["dispatched_date >= '" .$fiscal_year['2'] . "'"] = NULL;
				$where_bill_start_date["stockyard_id"] = $stockyard->stockyard_id;
				...
				$bill_no = $this->sparepart_order->get_new_invoice_no($stockyard->stockyard_id);
				$dispatched_date_nepali = $result->dispatched_date_nepali;
			...
			foreach ($grid_data as $key => $value) 
			{
				if($value['id']){
					unset($post['data']);
					$success = $this->update_order($value,$bill_no,$post,$stockyard->stockyard_id);
		...
		public function generate_bill($bill_no = NULL,$dispatched_date_nepali = NULL)
		{
			...
			if($rows)
			{
				if($rows[0]->stockyard_id){
					$where['id'] = $rows[0]->stockyard_id;
				}else{
					$where['id'] = 1;
				}
				$stockyard = $this->stockyard_model->find($where);
				// print_r($stockyard);
				$stockyard_name_array = explode(' ', $stockyard->name);
				if(count($stockyard_name_array) >= 3){
					$count = 3;
				}else{
					$count = count($stockyard_name_array);
				}
				$stockyard_abbr = '';
				for ($i=0; $i < $count; $i++) { 
					$stockyard_abbr .= substr($stockyard_name_array[$i], 0,1);
				}
				// print_r($stockyard_abbr);
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('A1',$stockyard->name);
				$objPHPExcel->getActiveSheet()->SetCellValue('A2','SPAREPARTS LOGISTIC DIVISION');
				$objPHPExcel->getActiveSheet()->SetCellValue('A3',$stockyard->location);
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('A5','Bill No: '. $stockyard_abbr .'-'.$bill_no);
		...
		public function delete_picklist_item()
		{
			...
			$picklist_detail = $this->picklist_model->find(array('id'=>$id));

			// if($picklist_detail->is_billed){
				$order_detail = $this->sparepart_order_model->find(array('id'=>$picklist_detail->order_id));
				$remaining_quantity = $order_detail->order_quantity - $order_detail->received_quantity - $order_detail->cancle_quantity;

				if($remaining_quantity <= 0){
					$msg = 'This item is already billed';           
					$success = FALSE;  
				}else{
					$data['id'] = $this->input->post('id');
					$success = $this->picklist_model->delete($data['id']);

					if($order_detail->order_quantity == $remaining_quantity){
						$success = $this->sparepart_order_model->delete($order_detail->id);
					}else{
						$order_detail->cancle_quantity += $remaining_quantity;
						$success = $this->sparepart_order_model->update($order_detail->id, $order_detail);
					}
					$msg = $remaining_quantity . ' item is deleted from order';
					$success = TRUE;
				}
			...
			// 		$msg = 'Sparepart removed from order';
			// 		$success = TRUE;
			// 	}
			// 	else
			// 	{
		...
		public function update_order($value,$bill_no,$post,$stockyard_id)
		{
			$total_credit = 0;
			//get order
			...
			if($success)
			{
				...
				$dispatch['stockyard_id'] = $stockyard_id;
				...
				$stock_update['quantity'] = $stock->quantity - $dispatched;
	...
	public function print_picklist_group($picklist_group)
	{
		...
		$this->db->order_by('part_code');
		$group = 'sparepart_id, part_code, name, latest_part_code, dealer_id, dealer_name, location, picker_name, first_name, middle_name, last_name, ordered_part_code, ordered_location';
	...
	public function export_backlog()
	{
		// echo '<pre>';
		// print_r($this->input->post());
		// echo '<br>';
		$dealer_id = $this->input->post('dealer_id');
		$fields = 'dealer_name, part_code, part_name as name, backorder AS total_backorder, pi_number, order_no_concat as order_no,id, order_type, dispatch_mode, dealer_price';
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('D1','Remaining Quantity');
        $objPHPExcel->getActiveSheet()->SetCellValue('E1','Order No');
        $objPHPExcel->getActiveSheet()->SetCellValue('F1','PI Number');
        $objPHPExcel->getActiveSheet()->SetCellValue('G1','Order Type');
        $objPHPExcel->getActiveSheet()->SetCellValue('H1','Dispatch Mode');
        $objPHPExcel->getActiveSheet()->SetCellValue('I1','Dealer Price');
        ...
        {   
            ...
            $col++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->total_backorder);
            $col++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->order_no);
            $col++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->pi_number);
            $col++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->order_type);
            $col++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->dispatch_mode);
            $col++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->dealer_price);
            $col++;      
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->dealer_price * $value->total_backorder);
            $col++;      
        ...
        header("Pragma: public");
        header("Content-Type: application/force-download");
        header("Content-Disposition: attachment;filename=Spareparts Backlog.xls");
        header("Content-Transfer-Encoding: binary ");
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
        ob_end_clean();
        $objWriter->save('php://output');
	...
	public function export_decision_sheet()
	{
		// echo '<pre>';
		// print_r($this->input->post());
		// echo '<br>';
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('I1','Dealer Price');
        $objPHPExcel->getActiveSheet()->SetCellValue('J1','Total Amount');
        $objPHPExcel->getActiveSheet()->SetCellValue('K1','Stock Quantity');
        $objPHPExcel->getActiveSheet()->SetCellValue('L1','Min Bill Quantity');
        $objPHPExcel->getActiveSheet()->SetCellValue('M1','Min Bill Amount');
        ...
        {   
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->dealer_price * $value->total_backorder);
            $col++;        
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->current_stock);
            $col++;      
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->min_bill_quantity);
            $col++;      
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->min_bill_amount);
            $col++;      
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $value->location);
            $col++;      
