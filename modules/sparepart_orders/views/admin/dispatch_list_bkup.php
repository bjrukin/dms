...
$('#scan_barcode').keypress(function(e){
	if(e.which == 13) 
	{
		...
		var dealer_id = $('#dealer_list').val();
		var order_no = $('#order_list').val();
		$('#dealer_id_bill').val(dealer_id);
		$('#order_no_bill').val(order_no);
		$.post('<?php echo site_url('sparepart_orders/get_barcode_values'); ?>',{code:code,dealer_id:dealer_id,order_no:order_no},function(result){
			if (result.success == true) 
			{
				$("#jqxGridPiList").jqxGrid('clear')
				$.each(result.dispatch_list,function(i,v){								
					datarow = {
						'name':v.name,
						'part_code':v.part_code,
						'dispatch_quantity' : v.total_dispatched,
						'price' : v.price
					};
					$("#jqxGridPiList").jqxGrid('addrow', null, datarow);
				});
