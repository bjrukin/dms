		...
		<div class="row">
			...
			<div class="col-xs-12 connectedSortable">
				...
				<div id='jqxTabs'>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-control">
									<button class="btn btn-xs btn-flat btn-primary" id="export_range_btn">Export Multiple PI</button>
								</div>
							</div>
					...
					<div>
						<div class="row">
							<div  class='grid-toolbar' id="jqxGridDealer_backlogToolbar">
								<button type="button" class="btn btn-primary btn-flat btn-xs" id="exportBacklogBtn">Export</button>
							</div>
							<div class="col-md-12">
					...
					<div>
						<div class="row">
							<div  class='grid-toolbar' id="jqxGridDecisionSheetToolbar">
								<a href="<?php echo site_url('sparepart_orders/export_decision_sheet')?>" class="btn btn-primary btn-flat btn-xs" id="exportDecisionSheetBtn">Export</a>
							</div>
							<div class="col-md-12">
								<div id="jqxGridDecisionSheet"></div>
							</div>
						</div>
					</div>
				</div>
...
<div id="jqxPopupWindowUnavailable_list">
	<div class='jqxExpander-custom-div'>
		<span class='popup_title'><?php echo "Incorrect Parts List"//lang('cancel_order');?></span>
	</div>
	<div class="row">
		<div class="col-md-12">
			<div id="unavailable">				
...
<div id="jqxPopupWindowPicking_list">
	...
	<div class="form_fields_area">
		<form action="<?php echo site_url('admin/sparepart_orders/generate_picking_list') ?>" method="POST">
			...
			<input type="hidden" name="dispatched_date" id="dispatched_date">
			<input type="hidden" name="picklist_status" id="picklist_status">
...
<div id="jqxPopupWindowExportRangePI">
	<div class='jqxExpander-custom-div'>
		<span class='popup_title'><?php echo "Picklist"; ?></span>
	</div>
	<div class="form_fields_area">
		<form action="<?php echo site_url('admin/sparepart_orders/pi_range_export') ?>" method="POST">
			<label for ='pi_range_start'>Start Date</label>
			<div id="pi_range_start" class="date_box" name="pi_range_start"></div>
			<label for ='pi_range_end'>End Date</label>
			<div id="pi_range_end" class="date_box" name="pi_range_end"></div>
			<div class="row">
				<div class="col-md-12">
					<button type="submit" class="btn btn-success btn-sm" ><?php echo "Generate" ?></button>
					<button type="button" class="btn btn-default btn-sm" id="jqxExportRangePiCancelButton"><?php echo lang('general_cancel'); ?></button>
				</div>
			</div>
		</form>
	</div>
...
<div id="jqxPopupWindowExportBacklog">
	<div class='jqxExpander-custom-div'>
		<span class='popup_title'><?php echo "Backlog"; ?></span>
	</div>
	<div class="form_fields_area">
		<form action="<?php echo site_url('admin/sparepart_orders/export_backlog') ?>" method="POST">
			<div class="col-md-2">
				<td><label for='dealer_id_datewise'>Dealer</label></td>
			</div>
			<div class="col-md-10">
				<td><div id='dealer_id_backlog' name='dealer_id'></div></td>
			</div>
			<div class="row">
				<div class="col-md-12">
					<button type="submit" class="btn btn-success btn-sm" ><?php echo "Generate" ?></button>
					<button type="button" class="btn btn-default btn-sm" id="jqxBacklogExportCancelButton"><?php echo lang('general_cancel'); ?></button>
				</div>
			</div>
		</form>
	</div>
	...
	$("#jqxGridSparepart_order").jqxGrid({
		...
		{ text: '<?php echo lang("pi_generated_date_time"); ?>',datafield: 'pi_generated_date_time',width: 90,filterable: true,renderer: gridColumnsRenderer, columntype: 'date', filtertype: 'range', cellsformat:  formatString_yyyy_MM_dd},
	...
	$("#jqxPopupWindow_dispatch_item_list").jqxWindow({
		theme: theme,
		width: '99%',
		maxWidth: '99%',
		height: '90%',
	...
	$(document).on('click','#exportBacklogBtn',function(){
		openPopupWindow('jqxPopupWindowExportBacklog', '<?php echo lang("general_add") . "&nbsp;" . $header; ?>');
	});

	$("#jqxBacklogExportCancelButton").on('click', function () {
		reset_Datewise_form();
		$('#jqxPopupWindowExportBacklog').jqxWindow('close');
	});

	// initialize the popup window
    $("#jqxPopupWindowExportBacklog").jqxWindow({ 
        theme: theme,
        width: 500,
        maxWidth: 500,
        height: 300,  
...
<script type="text/javascript">
	$(function(){
		...
		$("#jqxGridPI_indexed").jqxGrid({
			...
			{ text: '<?php echo lang("dispatch_mode"); ?>',datafield: 'dispatch_mode',width: 100 , editable:false,filterable: true,renderer: gridColumnsRenderer },
			...
			{ text: '<?php echo 'Pick Count'; ?>',datafield: 'pick_count',width: 150 , editable:false,filterable: true,
				cellsrenderer: function (index) {
							var rows = $("#jqxGridPI_indexed").jqxGrid('getrowdata', index);
							var e = '';
							if(rows.pick_count == null || rows.pick_count == ''){
								e = 'Not Generated';
							}else{
								e = rows.pick_count;
							}
	...
	$("#jqxPopupWindowExportRangePI").jqxWindow({ 
		theme: theme,
		width: '40%',
		maxWidth: '40%',
		height: '40%',  
	...
	$("#export_range_btn").on('click', function () {
		$('#jqxPopupWindowExportRangePI').jqxWindow('open');
	});
	
	$("#jqxExportRangePiCancelButton").on('click', function () {
		$('#jqxPopupWindowExportRangePI').jqxWindow('close');
	});

	// Back Order Grid
	var backlogDataSource =
	{
		datatype: "json",
		datafields: [
		{ name: 'id', type: 'number' },
        ...
        beforeprocessing: function (data) {
        	backlogDataSource.totalrecords = data.total;
        },
	    // update the grid and send a request to the server.
	    filter: function () {
	...
	$("#jqxGridBacklogs").jqxGrid({
		...
		{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
		<?php
		if($this->session->userdata('id') == 434)
		{
		?>
			{
				text: 'Action', datafield: 'action', width:75, sortable:false,filterable:false, pinned:true, align: 'center' , cellsalign: 'center', cellclassname: 'grid-column-center', 
				cellsrenderer: function (index) {
					var row =  $("#jqxGridBacklogs").jqxGrid('getrowdata', index);
					var e = '';
					var e = '<a href="javascript:void(0)" onclick="delete_backlog(' + index + '); return false;" title="Delete"><i class="fa fa-trash"></i></a>';
					return '<div style="text-align: center; margin-top: 8px;">' + e + '</div>';
				}
			},	
		<?php
		...
		{ text: '<?php echo lang("total_backorder"); ?>',datafield: 'total_backorder',width: 150,filterable: false,renderer: gridColumnsRenderer,aggregates: ['sum'] },	  
	...
	var decisionSheetDataSource =
	{
		...
		{ name: 'min_bill_quantity', type: 'number' },
		{ name: 'min_bill_amount', type: 'number' },
        ...
        beforeprocessing: function (data) {
        	decisionSheetDataSource.totalrecords = data.total;
        },
	    // update the grid and send a request to the server.
	    filter: function () {
	    	$("#jqxGridDecisionSheet").jqxGrid('updatebounddata', 'filter');
	    },
	    // update the grid and send a request to the server.
	    sort: function () {
	    	$("#jqxGridDecisionSheet").jqxGrid('updatebounddata', 'sort');
	    },
	    processdata: function(data) {
	    }
	};
	
	$("#jqxGridDecisionSheet").jqxGrid({
		theme: theme,
		width: '100%',
		height: gridHeight,
		source: decisionSheetDataSource,
		altrows: true,
		pageable: true,
		sortable: true,
		rowsheight: 30,
		...
		rendertoolbar: function (toolbar) {
			var container = $("<div style='margin: 5px; height:50px'></div>");
			container.append($('#jqxGridDecisionSheetToolbar').html());
			toolbar.append(container);
		},
		columns: [
		{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
		...
		?>
			{
				text: 'Action', datafield: 'action', width:75, sortable:false,filterable:false, pinned:true, align: 'center' , cellsalign: 'center', cellclassname: 'grid-column-center', 
				cellsrenderer: function (index) {
					var row =  $("#jqxGridDecisionSheet").jqxGrid('getrowdata', index);
					var e = '';
					var e = '<a href="javascript:void(0)" onclick="delete_backlog(' + index + '); return false;" title="Delete"><i class="fa fa-trash"></i></a>';
					return '<div style="text-align: center; margin-top: 8px;">' + e + '</div>';
				}
		...
		{ text: '<?php echo lang("current_stock"); ?>',datafield: 'current_stock',width: 150,filterable: true, renderer: gridColumnsRenderer },	  
		{ text: '<?php echo lang("min_bill_quantity"); ?>',datafield: 'min_bill_quantity',width: 150,filterable: true, renderer: gridColumnsRenderer },	  
		{ text: '<?php echo lang("min_bill_amount"); ?>',datafield: 'min_bill_amount',width: 150,filterable: true, renderer: gridColumnsRenderer },	  
	...
	$("[data-toggle='offcanvas']").click(function(e) {
		e.preventDefault();
		setTimeout(function() {$("#jqxGridDecisionSheet").jqxGrid('refresh');}, 500);
	});
});
...
function generate_picking_list(index)
{
	...
	$.ajax({
		type: "POST",
		url: '<?php echo site_url("admin/sparepart_orders/get_dispatch_date"); ?>',
		data: {order_no : row.order_no,pi_number : row.pi_number},
		success: function (result) {
				$('#jqxPopupWindowPicking_list').unblock();
				var a = JSON.parse(result);
				var dispatched_date = a.dispatched_date;
				var picklist = a.picklist;
				$('#dispatched_date').val(dispatched_date);	
				$('#picklist_status').val(picklist);	
...
<div id="jqxPopupWindow_dispatch_item_list">
	<div class='jqxExpander-custom-div'>
		<span class='popup_title'>Dispatch List Items</span>
	</div>
	<div class="row">
		<div class="col-md-12">
			<div id="jqxGrid_dispatch_items"></div>
		</div>
	</div>
</div>

<script type="text/javascript">
	function display_items(index)
	{
		var rows =  $("#jqxGridDispatchList").jqxGrid('getrowdata', index);
		...
		var sparepart_dispatchitemlist =
		{
			datatype: "json",
			datafields: [
			{ name: 'order_no', type: 'number' },			
			...
			url: '<?php echo site_url("admin/sparepart_orders/order_dispatch_detail")?>',
			data: {bill_no:rows.bill_no, dispatched_date:rows.dispatched_date},
			pagesize: defaultPageSize,
			root: 'rows',
			id : 'id',
			cache: true,
			...
			beforeprocessing: function (data) {
				sparepart_dispatchitemlist.totalrecords = data.total;
			},
			filter: function () {

			},
			sort: function () {
				$("#jqxGrid_dispatch_items").jqxGrid('updatebounddata', 'sort');
			},
			processdata: function(data) {
			}
		};

		$("#jqxGrid_dispatch_items").jqxGrid({
			theme: theme,
			width: '100%',
			// height: gridHeight,
			source: sparepart_dispatchitemlist,
			altrows: true,
			pageable: true,
			sortable: true,
			rowsheight: 30,
			...
			{ text: '<?php echo lang("dispatch_quantity"); ?>',datafield: 'dispatched_quantity',width: 150,filterable: true,renderer: gridColumnsRenderer, aggregates: ['sum']  },
			{ text: '<?php echo lang("price"); ?>',datafield: 'price',width: 150,filterable: true,renderer: gridColumnsRenderer},
			{ text: '<?php echo lang("total_amount"); ?>',datafield: 'total_amount',width: 150,filterable: true,renderer: gridColumnsRenderer, aggregates: ['sum'] },
			],
			rendergridrows: function (result) {
				return result.data;
			}
		});
		openPopupWindow('jqxPopupWindow_dispatch_item_list', 'Dispatch List Items');
	}
</script>
<script type="text/javascript">
	function delete_backlog(index){
			if(confirm('Are you sure'))
			{
				var rows = $("#jqxGridBacklogs").jqxGrid('getrowdata', index);
				// console.log(rows);
				// console.log(rows.id);

				$('#jqxGridBacklogs').block({ 
					message: '<span>Processing your request. Please be patient.</span>',
					css: { 
						width                   : '75%',
						border                  : 'none', 
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("admin/sparepart_orders/delete_backlog"); ?>',
					data: {id : rows.id, qty : rows.total_backorder},
					success: function (result) {
						var result = eval('('+result+')');
						if (result.success) {
							$('#jqxGridBacklogs').unblock();
							$('#jqxGridBacklogs').jqxGrid('updatebounddata');
						}
					}
				});
			}
