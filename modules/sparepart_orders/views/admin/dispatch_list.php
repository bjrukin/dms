					...
					<fieldset>
						...
						<div id="order-form">
							<div class="row">
								...
								<div class="col-sm-2">
									<div id="dealer_list_order" name="dealer_list_order"></div>												
								</div>							
							...
							<div class="row">
								...
								<div class="col-sm-2"><label>Bill no:</label></div>
								<div class="col-sm-2"><input type="text" class="form-control" name="bill_no" id="bill_no"></div>
							</div>
						</div>
					</fieldset>
				</div>
	...
	<script language="javascript" type="text/javascript">
		$(function(){
			...
			$("#jqxGridPiList").jqxGrid({
				...
				{ text: '<?php echo lang("dispatch_quantity"); ?>',datafield: 'dispatch_quantity',width:200, filterable: false,cellsalign: 'right',renderer: gridColumnsRenderer,aggregates: ['sum']},
				{
					...
					cellsrenderer: function (index) {
						var vor = parseFloat($('#vor_percentage_bill').val() || 0);
						var rows = $("#jqxGridPiList").jqxGrid('getrowdata', index);
						var e = (rows.dealer_price + (rows.dealer_price * vor/100)) * rows.dispatch_quantity;
			...
			$('#dealer_list_order').on('change', function (event) {
				var dealer_id = $('#dealer_list_order').val();
		...
		$('#scan_barcode').keypress(function(e){
			if(e.which == 13) 
			{
				...
				if(duplicate == 0){
					var dealer_id = $('#dealer_list_order').val();
					$.post('<?php echo site_url('admin/sparepart_orders/get_barcode_values'); ?>',{code:code,dealer_id:dealer_id},function(result){
						if (result.success == true) 
						{
							...
							var dispatch_ListDataSource =
							{
								...
								url: '<?php echo site_url("admin/sparepart_orders/dispatch_spareparts_list_json"); ?>',
								// pagesize: defaultPageSize,
								data : {dealer_id:result.dealer_id},
								root: 'rows',
								id : 'id',
								cache: true					
							};
		...
		$('#jqxGridPiList').on('cellvaluechanged', function (event) {
			...
			$.post('<?php echo site_url('admin/sparepart_orders/update_dispatch_quantity') ?>',{id : rowdata.id, dispatch_quantity:rowdata.dispatch_quantity,order_id : rowdata.order_id,picklist_no:rowdata.picklist_no},function(result)
			{
				if(result.success)
				{
					...
					alert(result.msg);
					$("#jqxGridPiList").jqxGrid('setcellvalue', rowBoundIndex, "dispatch_quantity", "1");
				}
		...
		$('#save_bill').on('click',function(){
			...
			$.ajax({
				...
				data: {data : data, vor_percentage:vor_percentage,discount_percentage:discount_percentage,billed_date:billed_date,remarks:remarks,bill_no:bill_no},
		...
		$('#generate_bill').click(function(){
			// location.reload();
			$('#generate_bill').html('<a href="<?php echo site_url('sparepart_orders/dispatch_list')?>" class="btn btn-success"><i class="fa fa-refresh"></i> Refresh</a>')
		});
