...
<div class="content-wrapper">
	...
	<section class="content">
        ...
        <!-- /.row -->
        <div class="row" id='opening-table-box' style="display:none">
            <div class="col-md-12 col-sm-6 col-xs-12">
                <div class="box box-solid">
                    <div class="box-body">
                        <h3>Opening Booking</h3>

                        <div id ="report-table-next"></div>
                    </div>
                </div>
            </div><!-- /.col -->
        </div>
...
function generateReport() {
 ...
 //    });
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo site_url("admin/crm-reports/get_booking_report"); ?>',
        data: data,
        success: function (result) {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo site_url("admin/crm-reports/booking_report_json_data"); ?>',
                data: data,
                ...
                success: function (result) {
                    ...
                    var result = eval('('+result+')');
                    if (result.success && result.total >= 1) {
                         $("#report-table-next").pivotUI(result.data, {
                            aggregatorName: "Count",
                            rendererName: "Table",
                        });
                        $('#opening-table-box').show();
                        $('#jqxButtonCopyToClipboard').show();
                    } else {
                        $('#opening-table-box').hide();
                        $('#jqxButtonCopyToClipboard').hide();
                        alert('No Records for generating Reports');
                    }
