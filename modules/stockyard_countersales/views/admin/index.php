	...
	$(function(){
		...
		$("#jqxGridCounter_sale").jqxGrid({
			...
			{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
			{
				...
				cellsrenderer: function (index,b,c,d,e,rows) {
					var view = cr = mi = bill = gate = returnvalue = generate_bill = '' ;
					...
					generate_bill = ' | <a href="javascript:void(0)" onclick="print_bill(' + index + '); return false;" title="Generate Bill"><i class="fa fa-book"></i></a>';
			...
			{ text: '<?php echo lang("counter_sales_id"); ?>',datafield: 'countersale_no',width: 150,filterable: true,renderer: gridColumnsRenderer, cellclassname: cellclassrenderer, cellsrenderer: function(a,b,value,d,e,row) {
				console.log(value);
		...
		CounterSalesRequest = $("#materialCounterJqxgrid").jqxGrid(
		{
			...
			{ 
				...
				{ text: '<?php echo lang("sparepart_id")?>', datafield: 'sparepart_id', editable : false, hidden:true },
		...
		$('#add_part_submit').click(function(){
			...
			var validationResult = function (isValid) {
				if (isValid) {
					...
					form.sparepart_id = selected_data.originalItem.id;
...
<script type="text/javascript">
	...
	function editCounter_saleRecord(index){
		...
		if (row) {
			...
			$('#issueCountersales-countersales_no_display').text("CI-"+ (row.countersale_no).pad(5) );
			<?php if(is_group(CG_SPAREPART_OUTLET)){?>
				$('#vro').val(row.vro);
			<?php }?>
	...
	function saveCounter_saleRecord(){
		...
		$.ajax({
			type:       "POST",
			url: 		'<?php echo site_url("admin/stockyard_countersales/save_counter_sales"); ?>',
			data:       {
...
<script type="text/javascript">
	$('#add_part_quantity, #add_part_name').on('change',function(){
		...
		$.post('<?php echo base_url("stockyard_countersales/check_stock")?>',data,function(result) {
			if(!result.success){
...
<script type="text/javascript">
	function print_bill(index) {
		var row =  $("#jqxGridCounter_sale").jqxGrid('getrowdata', index);

		var url = base_url+"stockyard_countersales/invoice?id="+row.id+"&counter_sales_no="+row.countersale_no;
