   ...
   <div class='jqxExpander-custom-div'>
    ...
    <div class="form_fields_area">
        <?php echo form_open('', array('id' =>'form-billing_details', 'onsubmit' => 'return false')); ?>
        	<input type = "hidden" name = "detail_id" id = "billing_details_id"/>
            ...
            <table class="form-table">
				<tr>
					...
					<!-- <td><input id='bill_no' class='text_input' name='bill_no'></td> -->
					<td><label for='billed_date'><?php echo lang('billed_date')?></label></td>
					<td><div id='billed_date' class='date_box' name='billed_date'></div></td>
			...
			<table>
                ...
                <tr>
                    <th colspan="2">
                        <button type="button" class="btn btn-success btn-xs btn-flat" id="jqxBilling_detailSubmitButton"><?php echo lang('general_save'); ?></button>
...
<div id="jqxPopupWindowView_Billing_detail">
   <div class='jqxExpander-custom-div'>
        <span class='popup_title' id="window_poptup_title">View</span>
    </div>
    <div class="form_fields_area">
        <?php echo form_open('', array('id' =>'form-billing_details', 'onsubmit' => 'return false')); ?>
        	<input type = "hidden" name = "detail_id" id = "view_billing_details_id"/>
			<input type="hidden" name="dealer_id" id="view_dealer_id" value="<?php echo $dealer_id?>">
            <table class="form-table">
				<tr>
					<td><label for='bill_no'><?php echo lang('bill_no')?></label></td>
					<td><div id='view_bill_no'></div></td>
					<td><label for='billed_date'><?php echo lang('billed_date')?></label></td>
					<td><div id='view_billed_date'></div></td>
				</tr>
				<tr>
					<td><label for='billed_to'><?php echo lang('billed_to')?></label></td>
					<td><div id='billed_to'></div></td>
				</tr>
				<tr>
					<td><label for='status'><?php echo lang('status')?></label></td>
					<td><div id='view_status'></td>
				</tr>
			</table>
			<div class="col-md-12">
				<div id="jqxGridViewList"></div>
			</div>
			<br>
			<table>
                <tr>
                    <th colspan="2">
                        <button type="button" class="btn btn-default btn-xs btn-flat" id="jqxView_Billing_detailCancelButton"><?php echo lang('general_cancel'); ?></button>
                        <button type="button" class="btn btn-primary btn-xs btn-flat" id="jqxView_Billing_detailGenerateButton">Generate</button>
                    </th>
                </tr>
...
$(function(){
	...
	var sparepart_dealerDataSource = {
		url : '<?php echo site_url("admin/billing_lists/get_spareparts_dealers_combo_json/1"); ?>',
	...
	var billing_detailsDataSource =
	{
		...
		datafields: [
			...
			{ name: 'billed_to_person', type: 'string' },
			{ name: 'is_billed', type: 'number' },
			
        ],
		url: '<?php echo site_url("admin/billing_details/json"); ?>',
	...
	$("#jqxGridBilling_detail").jqxGrid({
		...
		columns: [
				...
				cellsrenderer: function (index) {
					...
					if(!row.is_billed){
						if(row.status == 'PENDING'){
							var e = '<a href="javascript:void(0)" onclick="editBilling_detailRecord(' + index + '); return false;" title="Edit"><i class="fa fa-edit"></i></a> | ';
						}
						console.log(row.is_billed);
						var v = '<a href="javascript:void(0)" onclick="viewBilling_detailRecord(' + index + '); return false;" title="View"><i class="fa fa-eye"></i></a>';
					}

					var p = '';
					if(row.is_billed){
						p = '<a href="<?php echo site_url()?>/admin/billing_details/generate_bill/'+ row.id +'" target="blank" title="print"><i class="fa fa-print"></i></a> | ';
					}
					return '<div style="text-align: center; margin-top: 8px;">' + e + p + v + '</div>';
			...
			{ text: '<?php echo lang("billed_to"); ?>',datafield: 'billed_to_person',width: 250,filterable: true,renderer: gridColumnsRenderer },
	...
	$("#jqxGridPiList_item").jqxGrid({		
		...
		showstatusbar: true,
		ready: function () {
			var rowsCount = $("#jqxGridPiList_item").jqxGrid("getrows").length;
			for (var i = 0; i < rowsCount; i++) {
				var currentQuantity = $("#jqxGridPiList_item").jqxGrid('getcellvalue', i, "dispatched_quantity");
				var currentPrice = $("#jqxGridPiList_item").jqxGrid('getcellvalue', i, "price");
				var currentTotal = currentQuantity * currentPrice;
				$("#jqxGridPiList_item").jqxGrid('setcellvalue', i, "total", currentTotal.toFixed(2));
			}
		},
		columns: [
		{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false,editable:false},
		{
			...
			cellsrenderer: function (index, row, columnfield, value, defaulthtml, columnproperties) {
				var rows = $("#jqxGridPiList_item").jqxGrid('getrowdata', index);
				var e = '';
		...
		{ text: '<?php echo lang("quantity"); ?>',datafield: 'quantity',width:200, filterable: false,cellsalign: 'right',renderer: gridColumnsRenderer,aggregates: ['sum'] },
		{
			...
			cellsrenderer: function (index) {
				var row = $("#jqxGridPiList_item").jqxGrid('getrowdata', index);
	...
	$("[data-toggle='offcanvas']").click(function(e) {
		e.preventDefault();
		setTimeout(function() {$("#jqxGridPiList_item").jqxGrid('refresh');}, 500);
	});

	$(document).on('click','#jqxGridPiList_itemFilterClear', function () { 
		$('#jqxGridPiList_item').jqxGrid('clearfilters');
	});

	$(document).on('click','#jqxGridBilling_detailInsert', function () { 
		$('#jqxGridPiList_item').jqxGrid('clear');
    ...
    $("#jqxGridViewList").jqxGrid({		
		...
		{ text: '<?php echo lang("quantity"); ?>',datafield: 'quantity',width:200, filterable: false,cellsalign: 'right',renderer: gridColumnsRenderer,aggregates: ['sum'] },
		{
			text: 'Total Amount', datafield: 'total', sortable:false , width:200, filterable:false, align: 'center' , editable:false, cellsalign: 'right', 
			cellsrenderer: function (index) {
				var row = $("#jqxGridViewList").jqxGrid('getrowdata', index);
				var e = row.price * row.quantity;
				return '<div style="text-align: right; margin-top: 8px;">' + e.toLocaleString('en-IN', {minimumFractionDigits : 2}) + '</div>';
			},
			aggregates: [{ 'Sum':
    ...
    // initialize the popup window
    $("#jqxPopupWindowView_Billing_detail").jqxWindow({ 
        theme: theme,
        width: '75%',
        maxWidth: '75%',
        height: '75%',  
    ...
    $("#jqxPopupWindowView_Billing_detail").on('close', function () {
        // reset_form_billing_details();
    });

    $("#jqxView_Billing_detailCancelButton").on('click', function () {
        // reset_form_billing_details();
        $('#jqxPopupWindowView_Billing_detail').jqxWindow('close');
    });
    ...
    $("#jqxBilling_detailSubmitButton").on('click', function () {
        saveBilling_detailRecord();
        /*
        ...
        $('#form-billing_details').jqxValidator('validate', validationResult);
        */
    });
    ...
    $('#scan_barcode_no_order').keypress(function(e){
		if(e.which == 13) 
		{
			...
			$.post('<?php echo site_url('admin/billing_details/set_barcode_values'); ?>',{code:code,dealer_id:dealer_id,ledger_id:ledger_id},function(result){
				...
				{
					$("#jqxGridPiList_item").jqxGrid('addrow', null, result.data);
...
function viewBilling_detailRecord(index) {
	var row =  $("#jqxGridBilling_detail").jqxGrid('getrowdata', index);
  	if (row) {
  		$('#jqxGridViewList').jqxGrid('clear');
  		$.post('<?php echo site_url("admin/billing_lists/get_list_json")?>',{id:row.id},function(result){
  			$.each(result.rows,function(key,value){
  				$("#jqxGridViewList").jqxGrid('addrow', null, value);
  			});
  		},'json');
  		$('#view_billing_details_id').val(row.id);
		$('#view_dealer_id').html( row.dealer_id);
		$('#view_bill_no').html(row.bill_no);
		var date = new Date(row.billed_date);
		$('#view_billed_date').html(date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate());
		$('#view_billed_date_np').val(row.billed_date_np);
		$('#billed_to').html(row.billed_to);
		$('#view_billed_time').val(row.billed_time);
		$('#view_status').html(row.status);
		
        openPopupWindow('jqxPopupWindowView_Billing_detail', 'View');
    }
}

function saveBilling_detailRecord(){
    var data = $("#form-billing_details").serialize();
    var item_data = $('#jqxGridPiList_item').jqxGrid('getrows');
    var item_record = '';
...
<script type="text/javascript">
	function delete_order(index) {
		var row =  $("#jqxGridPiList_item").jqxGrid('getrowdata', index);
		if(row.id != null){
			if(confirm('Do you want to delete ' + row.part_code + '(' + row.part_name + ')')){
				$.post('<?php echo site_url("admin/billing_lists/delete")?>',{id:row.id},function(result){
					if(result.success){
						var rowid = $('#jqxGridPiList_item').jqxGrid('getrowid', index);
						$('#jqxGridPiList_item').jqxGrid('deleterow', rowid);
					}
				},'json');
			}
		}
...
<script type="text/javascript">
	$('#jqxView_Billing_detailGenerateButton').on('click', function () {
		var billing_details_id = $('#view_billing_details_id').val();

		$('#jqxPopupWindowView_Billing_detail').block({ 
	        message: '<span>Processing your request. Please be patient.</span>',
	        css: { 
	            width                   : '75%',
	            border                  : 'none', 
		...
		$.ajax({
		  method: "POST",
		  url: "<?php echo site_url('billing_details/save_bill')?>",
		  ...
		  success: function(result){
		  	...
		  	if(result.success){
		  		$('#jqxGridBilling_detail').jqxGrid('updatebounddata');
                $('#jqxPopupWindowView_Billing_detail').jqxWindow('close');
                var win = window.open('<?php echo site_url()?>/admin/billing_details/generate_bill/'+ billing_details_id);
				if (win) {
				    win.focus();
				} else {
		  	...
		  	$('#jqxPopupWindowView_Billing_detail').unblock();
		  }
		});
	});
</script>