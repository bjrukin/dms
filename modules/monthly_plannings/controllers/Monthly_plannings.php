...
//        for stock according to stockyard according to month
    ...
    public function get_report_json($year = NULL)
    {
        ...
        // for msil order
        $this->monthly_planning_model->_table = 'view_msil_orders';
        $fields = 'vehicle_name AS Model, variant_name AS Variant, color_id AS Color_id, year AS Year, month AS Month, total AS "ordered_quantity"';

        $this->db->group_by(array('vehicle_name','variant_name','color_id','year','month','total'));
        ...
        foreach ($vehicles as $index => $row){
            ...
            for($i = 1; $i <= 12; $i++){
                ...
                for($j = 0; $j < abs($total); $j++){
                    $vehicle_records[] = array('Model'=>$row->vehicle_name, 'Variant' => $row->variant_name, 'Color' => $row->color_name,'Month' => $i);
                }
            }
        }
    ...
    public function msil_pending_get_report()
    {
        $data['header']                 = 'MSIL Pending Report';
        $data['page']                   = $this->config->item('template_admin') . "pending_report";
        $data['module']                 = 'monthly_plannings';  
        $this->load->view($this->_container,$data);
    ...
    public function pending_json()
    {
        ...
        // $this->monthly_planning_model->_table = 'view_msil_grouped_pending_report';
        // search_params();
        // $this->db->where('sum <>',0);
        // $total = $this->monthly_planning_model->find_count();
        // paging('order_id');
        ...
        $select[] = 'varient_name as "Varient Name"';
        $select[] = 'vehicle_name as "Vehicle Name"';
        $select[] = 'firm_name as "Firm"';
        $select[] = 'color_name as "Color Name"';
        $select[] = 'sum';
        $select[] = 'month AS "Month"';
        $select[] = 'code as "Color Code"';
        $this->monthly_planning_model->_table = 'view_msil_grouped_pending_report';
        $this->db->where('sum <>',0); 
        if($year != null || $year != '' || !empty($year))
        $this->db->where('year',$year);       
        if($month != null || $month != '' || !empty($month))
        $this->db->where('month',$month);       
        $rows = $this->monthly_planning_model->findAll(null,$select);
        $datas = array();
        foreach ($rows as $key => $value) {
        ...
        echo json_encode(array('success'=>true, 'data' =>$datas, 'total' => count($datas)));
    }
