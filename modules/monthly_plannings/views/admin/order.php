...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1><?php echo lang('msil_order_planning'); ?></h1>
        <ol class="breadcrumb">
            <li><a href="#">Home</a></li>
        ...
        <div class="row">
            <div class="col-xs-12 connectedSortable">
                ...
                <form action="<?php echo base_url('monthly_plannings/generate_order') ?>" method="post" enctype="multipart/form-data">
                    ...
                    <button class="btn btn-primary btn-flat btn-xs">Generate</button>
                </form>
                <div id='jqxGridMonthly_planningToolbar' class='grid-toolbar'></div>
                                ...
                                <td><?php
                                    ...
                                    <td><?php
                                        if(is_array($records['transit'])){
                                            if(key_exists('rxl_custom', $records['transit']) && key_exists($row->vehicle_id, $records['transit']['rxl_custom']) && key_exists($row->variant_id, $records['transit']['rxl_custom'][$row->vehicle_id]) && key_exists($row->color_id, $records['transit']['rxl_custom'][$row->vehicle_id][$row->variant_id])){
                                                echo $records['transit']['rxl_custom'][$row->vehicle_id][$row->variant_id][$row->color_id]['count'];
                                            }else{
                                                echo 0;
                                            }
                                        }else{
                                        ...
                                        <td><?php 
                                            if(is_array($records['transit'])){
                                                if(key_exists('grp_custom', $records['transit']) && key_exists($row->vehicle_id, $records['transit']['grp_custom']) && key_exists($row->variant_id, $records['transit']['grp_custom'][$row->vehicle_id]) && key_exists($row->color_id, $records['transit']['grp_custom'][$row->vehicle_id][$row->variant_id])){
                                                    echo $records['transit']['grp_custom'][$row->vehicle_id][$row->variant_id][$row->color_id]['count'];
                                                }else{
                                                    echo 0;
                                                }
                                            }else{
                                    ...
                                    <form action="<?php echo site_url('monthly_plannings/excel_export_msil_order') ?>" method="post">
                                        <input type="hidden" name="export_year" id="export_year">
                                        <input type="hidden" name="export_month" id="export_month">
                                        <!-- <button class="btn btn-primary btn-flat btn-xs">Export</button>             -->
                                    </form>
                                    <!-- row -->
                                    <div class="row">
                                        <div class="col-xs-12 connectedSortable">
...
console.log(rowData);
$("#jqxTableOrder").jqxDataTable('updateRow', editRow, rowData);
});
                $("#dialog").on('close', function () {
                        // enable jqxDataTable.
                        $("#jqxTableOrder").jqxDataTable({ disabled: false });
                    });
                $("#dialog").jqxWindow({
                    resizable: false,
                    position: { left: $("#jqxTableOrder").offset().left + 300, top: $("#jqxTableOrder").offset().top + 135 },
            ...
            columns: [
            { text: 'Vehicle Name', dataField: 'Vehicle Name', width: 100 },
            { text: 'Variant', dataField: 'Variant', width: 100 },
            ...
            { text: 'GRP Custom', dataField: 'GRP Custom', width: 100 },
