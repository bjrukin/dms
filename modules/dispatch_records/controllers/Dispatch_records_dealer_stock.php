...
//        $index = array('dispatch_date', 'order_no', 'vehicle_name', 'variant', 'color', 'chass_no', 'engine_no', 'ait_reference_no', 'invoice_no', 'invoice_date', 'month', 'year');
        $index = array('dispatch_date', 'order_no', 'vehicle_name', 'variant', 'color', 'chass_no', 'engine_no', 'ait_reference_no', 'invoice_no', 'invoice_date', 'month', 'year','transit','indian_stock_yard','indian_custom','nepal_custom','stock_yard','reached_date','reached_date_nepali','dealer_name','repair_date');
        $raw_data = array();
        $data = array();
        $view_data = array();
...
//            print_r($vehicle);
           ...
           // echo '<pre>';print_r($stockyard_detail);
            if(count($stockyard_detail) > 0){
               ...
               $temp['dispatched_date_nepali'] = $value['reached_date'];
               $temp['reached_date_nepali'] = $value['reached_date'];
            }
            // print_r($temp);exit;
            ...
            $order_number = $this->dealer_order_model->get_All();
            $order_data['order_id'] = ($order_number[0]->order_id) + 1;
           ...
           // print_r($dealer);
           // exit;
            $order_data['dealer_id'] = $dealer[0]->id;
            // print_r($order_data);
            ...
            $dispatch_dealer['dealer_id'] = $dealer[0]->id;
