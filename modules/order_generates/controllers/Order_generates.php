...
class Order_generates extends Project_Controller
{
	public function __construct()
	{
    	parent::__construct();

    	control('Order Generates');

        $this->load->model('order_generates/order_generate_model');
        $this->lang->load('order_generates/order_generate');
    }

	public function index()
	{
		// Display Page
		$data['header'] = lang('order_generates');
		$data['page'] = $this->config->item('template_admin') . "index";
		$data['module'] = 'order_generates';
	...
	public function json()
	{
		...
		$total=$this->order_generate_model->find_count();
		
		paging('id');
		
		search_params();
		
		$rows=$this->order_generate_model->findAll();
