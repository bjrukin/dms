...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1><?php echo lang('menu_order_list'); ?></h1>
		<ol class="breadcrumb">
			<li><a href="#">Home</a></li>
			<li class="active"><?php echo lang('menu_order_list'); ?></li>
		</ol>
	</section>
	<!-- Main content -->
...
<div id="jqxPopupWindowEdit_Dispatch_month">
	<div class='jqxExpander-custom-div'>
		<span class='popup_title'>Edit Dispatch Month</span>
	</div>
	<div class="form_fields_area">
...
<div id="jqxPopupWindowLogistic_confirmaion">
	<div class='jqxExpander-custom-div'>
		<span class='popup_title'>Logistic Confirmation</span>
	</div>
	<div class="form_fields_area">
		<?php echo form_open('', array('id' => 'form-logistic_confirmation', 'onsubmit' => 'return false')); ?>
		<input type = "hidden" name = "order_id" id = "logistic_confirmation_id"/>
		<table class="form-table app-table">
			<tr>
				<td><label for="date">Date</label></td>
				<td><div id="logistic_confirmation_date" name="logistic_confirmation_date"></div></td>
			</tr>
			<tr>
			...
			<tr>
				<th colspan="3">
					<button type="button" class="btn btn-success btn-md btn-flat" id="jqxLogistic_confirmationSubmitButton"><?php echo lang('general_save'); ?></button>
					<button type="button" class="btn btn-default btn-md btn-flat" id="jqxLogistic_confirmationCancelButton"><?php echo lang('general_cancel'); ?></button>
				</th>
			</tr>
		</table>
		<?php echo form_close(); ?>
...
<script language="javascript" type="text/javascript">
	...
	$(function () {
		...
		$("#logistic_confirmation_date").jqxDateTimeInput({ width: '250px', height: '25px', formatString: "yyyy-MM-dd" });
...
var dealer_ordersDataSource =
{
	...
	url: '<?php echo site_url("admin/dealer_orders/get_order_status"); ?>',
...
$("#jqxGridDealer_order").jqxGrid({
	...
	{text: 'SN', width: 50, pinned: true, exportable: false, columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer, filterable: false},
	{
		...
		cellsrenderer: function (index) {
			...
			if (row.dispatch_id) {
				...
				e += '<a href="javascript:void(0)" onclick="logistic_confirmation(' + index + '); return false;" title="Logistic Confirmation"><i class="fa fa-clock-o" aria-hidden="true"></i></a>&nbsp';
				if(row.order_status == 'Display'){
					e += '<a href="javascript:void(0)" onclick="display2dealer(' + index + '); return false;" title="Allow Dealer to Retail"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>&nbsp'
				}
			}
			else
			{
	...
	{text: '<?php echo lang("payment_value"); ?>', datafield: 'payment_value', width: 170, filterable: true, renderer: gridColumnsRenderer, cellclassname : cellclassname},
	...
	{text: '<?php echo lang("customer_retail_date"); ?>', datafield: 'customer_retail_date', width: 170, filterable: true, renderer: gridColumnsRenderer, columntype: 'date', filtertype: 'range', cellsformat: formatString_yyyy_MM_dd, cellclassname:cellclassname},
	{ text: 'Challan Status',datafield: 'challan_status',width: 150,filterable: true,renderer: gridColumnsRenderer ,cellclassname:cellclassname},
	{ text: 'Challan Hold Location',datafield: 'location',width: 200,filterable: true,renderer: gridColumnsRenderer,cellclassname:cellclassname },
...
$("#jqxPopupWindowLogistic_confirmaion").jqxWindow({
	theme: theme,
	width: '40%',
	maxWidth: '40%',
	height: '50%',
...
$("#jqxPopupWindowLogistic_confirmaion").on('close', function () {
});
$("#jqxLogistic_confirmationCancelButton").on('click', function () {
	$('#jqxPopupWindowLogistic_confirmaion').jqxWindow('close');

});
$("#jqxLogistic_confirmationSubmitButton").on('click', function () {
	save_logistic_confirmation();
});
...
function cancel_dispatch(index) 
{
	...
	function logistic_confirmation(index) 
	{
		var row = $("#jqxGridDealer_order").jqxGrid('getrowdata', index);
		if(row)
		{
		...
		openPopupWindow('jqxPopupWindowLogistic_confirmaion', '<?php echo lang("general_edit") . "&nbsp;" . $header; ?>');
	}

	function save_logistic_confirmation()
	{
		var data = $("#form-logistic_confirmation").serialize();
		$('#jqxPopupWindowLogistic_confirmaion').block({
			message: '<span>Processing your request. Please be patient.</span>',
			css: {
				width: '75%',
				border: 'none',
		...
		$.ajax({
			type: "POST",
			url: '<?php echo site_url("admin/dealer_orders/save_logistic_confirmation"); ?>',
			data: data,
			success: function (result) {
				var result = eval('(' + result + ')');
				if (result.success == true) {
				...
				$('#jqxPopupWindowLogistic_confirmaion').unblock();
			}
		});

	}

	function display2dealer(index) {
		var row = $("#jqxGridDealer_order").jqxGrid('getrowdata', index);
		if(!confirm('Are you sure want to allow dealer to retail??'))
			return false;
		if(row){
			$('#jqxGridDealer_order').block({
				message: '<span>Processing your request. Please be patient.</span>',
				css: {
					width: '75%',
					border: 'none',
			...
			$.post('<?php echo site_url("admin/dealer_orders/change_display2dealer")?>',row,function(result){
				if(result.success == true){
					alert('Stock is billed');
					$('#jqxGridDealer_order').jqxGrid('updatebounddata');
				}else{
					alert('Vehicle is not billed. Please try again');
				}
