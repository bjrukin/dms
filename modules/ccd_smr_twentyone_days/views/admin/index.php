...
.cls-green { background-color: #69f579; }
.cls-yellow { background-color: #f4f569; }
.cls-blue { background-color: #fff; }
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1><?php echo lang('ccd_smr_twentyone_days'); ?></h1>
		<ol class="breadcrumb">
	        <li><a href="#">Home</a></li>
	        <li class="active"><?php echo lang('ccd_smr_twentyone_days'); ?></li>
      </ol>
	</section>
	<!-- Main content -->
	<section class="content">
		...
		<div class="row">
			<div class="col-xs-12 connectedSortable">
				...
				<div id='jqxGridCcd_smr_twentyone_dayToolbar' class='grid-toolbar'>
					<!-- <button type="button" class="btn btn-primary btn-flat btn-xs" id="jqxGridCcd_smr_twentyone_dayInsert"><?php echo lang('general_create'); ?></button> -->
					<button type="button" class="btn btn-danger btn-flat btn-xs" id="jqxGridCcd_smr_twentyone_dayFilterClear"><?php echo lang('general_clear'); ?></button>
				</div>
				<div id="jqxGridCcd_smr_twentyone_day"></div>
			</div><!-- /.col -->
		</div>
		<!-- /.row -->
	</section><!-- /.content -->
...
<div id="jqxPopupWindowCcd_smr_twentyone_day">
   <div class='jqxExpander-custom-div'>
        <span class='popup_title' id="window_poptup_title"></span>
    </div>
    <div class="form_fields_area">
        <?php echo form_open('', array('id' =>'form-ccd_smr_twentyone_days', 'onsubmit' => 'return false')); ?>
        	<input type = "hidden" name = "id" id = "ccd_smr_twentyone_days_id"/>
            <table class="form-table">
                ...
                <tr>
                    <th colspan="2">
                        <button type="button" class="btn btn-success btn-xs btn-flat" id="jqxCcd_smr_twentyone_daySubmitButton"><?php echo lang('general_save'); ?></button>
                        <button type="button" class="btn btn-default btn-xs btn-flat" id="jqxCcd_smr_twentyone_dayCancelButton"><?php echo lang('general_cancel'); ?></button>
                    </th>
                </tr>
...
$(function(){
	...
	var ccd_smr_twentyone_daysDataSource =
	{
		datatype: "json",
		datafields: [
			{ name: 'id', type: 'number' },
		...
		url: '<?php echo site_url("admin/ccd_smr_twentyone_days/json"); ?>',
        ...
        beforeprocessing: function (data) {
        	ccd_smr_twentyone_daysDataSource.totalrecords = data.total;
        },
	    // update the grid and send a request to the server.
	    filter: function () {
	    	$("#jqxGridCcd_smr_twentyone_day").jqxGrid('updatebounddata', 'filter');
	    },
	    // update the grid and send a request to the server.
	    sort: function () {
	    	$("#jqxGridCcd_smr_twentyone_day").jqxGrid('updatebounddata', 'sort');
	    },
	...
	$("#jqxGridCcd_smr_twentyone_day").jqxGrid({
		theme: theme,
		width: '100%',
		height: gridHeight,
		source: ccd_smr_twentyone_daysDataSource,
		altrows: true,
		pageable: true,
		sortable: true,
		rowsheight: 30,
		...
		rendertoolbar: function (toolbar) {
			...
			container.append($('#jqxGridCcd_smr_twentyone_dayToolbar').html());
		...
		columns: [
			{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
			{
				...
				cellsrenderer: function (index) {
					var e = '<a href="javascript:void(0)" onclick="editCcd_smr_twentyone_dayRecord(' + index + '); return false;" title="Edit"><i class="fa fa-edit"></i></a>';
	...
	$("[data-toggle='offcanvas']").click(function(e) {
	    e.preventDefault();
	    setTimeout(function() {$("#jqxGridCcd_smr_twentyone_day").jqxGrid('refresh');}, 500);
	});

	$(document).on('click','#jqxGridCcd_smr_twentyone_dayFilterClear', function () { 
		$('#jqxGridCcd_smr_twentyone_day').jqxGrid('clearfilters');
	});

	$(document).on('click','#jqxGridCcd_smr_twentyone_dayInsert', function () { 
		openPopupWindow('jqxPopupWindowCcd_smr_twentyone_day', '<?php echo lang("general_add")  . "&nbsp;" .  $header; ?>');
    });

	// initialize the popup window
    $("#jqxPopupWindowCcd_smr_twentyone_day").jqxWindow({ 
        theme: theme,
        width: '75%',
        maxWidth: '75%',
        height: '75%',  
    ...
    $("#jqxPopupWindowCcd_smr_twentyone_day").on('close', function () {
        reset_form_ccd_smr_twentyone_days();
    });

    $("#jqxCcd_smr_twentyone_dayCancelButton").on('click', function () {
        reset_form_ccd_smr_twentyone_days();
        $('#jqxPopupWindowCcd_smr_twentyone_day').jqxWindow('close');
    });

    /*$('#form-ccd_smr_twentyone_days').jqxValidator({
        hintType: 'label',
        ...
        rules: [
			...
			{ input: '#created_at', message: 'Required', action: 'blur', 
				rule: function(input) {
					val = $('#created_at').jqxNumberInput('val');
					return (val == '' || val == null || val == 0) ? false: true;
				}
			},
			...
			{ input: '#updated_at', message: 'Required', action: 'blur', 
				rule: function(input) {
					val = $('#updated_at').jqxNumberInput('val');
					return (val == '' || val == null || val == 0) ? false: true;
				}
			},
			...
			{ input: '#deleted_at', message: 'Required', action: 'blur', 
				rule: function(input) {
					val = $('#deleted_at').jqxNumberInput('val');
					return (val == '' || val == null || val == 0) ? false: true;
				}
			},
    ...
    $("#jqxCcd_smr_twentyone_daySubmitButton").on('click', function () {
        saveCcd_smr_twentyone_dayRecord();
        /*
        ...
        $('#form-ccd_smr_twentyone_days').jqxValidator('validate', validationResult);
        */
    });
});

function editCcd_smr_twentyone_dayRecord(index){
	...
	$("#appointment_date").jqxDateTimeInput({value: null});
	clear_combobox();
    var row =  $("#jqxGridCcd_smr_twentyone_day").jqxGrid('getrowdata', index);
  	if (row) {
  		$('#ccd_smr_twentyone_days_id').val(row.id);
  //       $('#created_by').jqxNumberInput('val', row.created_by);
		...
		// $('#created_at').jqxNumberInput('val', row.created_at);
		// $('#updated_at').jqxNumberInput('val', row.updated_at);
		// $('#deleted_at').jqxNumberInput('val', row.deleted_at);
		...
		// $('#appointment_date').jqxDateTimeInput('setDate', row.appointment_date);
		...
		$('#call_status').val(row.call_status);
  		...
  		$("#remark").val(row.remark);
        openPopupWindow('jqxPopupWindowCcd_smr_twentyone_day', '<?php echo lang("general_edit")  . "&nbsp;" .  $header; ?>');
    }
}

function saveCcd_smr_twentyone_dayRecord(){
    var data = $("#form-ccd_smr_twentyone_days").serialize();
	
	$('#jqxPopupWindowCcd_smr_twentyone_day').block({ 
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo site_url("admin/ccd_smr_twentyone_days/save"); ?>',
        data: data,
        success: function (result) {
            var result = eval('('+result+')');
            if (result.success) {
                reset_form_ccd_smr_twentyone_days();
                $('#jqxGridCcd_smr_twentyone_day').jqxGrid('updatebounddata');
                $('#jqxPopupWindowCcd_smr_twentyone_day').jqxWindow('close');
            }
            $('#jqxPopupWindowCcd_smr_twentyone_day').unblock();
        }
    });
}

function reset_form_ccd_smr_twentyone_days(){
	
	$('#ccd_smr_twentyone_days_id').val('');
	$("#remark").val('');
	$("#call_status").jqxComboBox('clearSelection'); 
	$("#call_status").val('');
	$("#call_connect_retail_type").jqxComboBox('clearSelection'); 
    ...
    $('#form-ccd_smr_twentyone_days')[0].reset();
}
