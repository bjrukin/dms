...
<div id="jqxPopupWindowPurchase_invoice">
	...
	<div class="form_fields_area">
		...
		<input type = "hidden" name = "purchase_invoice_serial" id = "purchase_invoice_serial"/>
						...
						<div class="row">
							<div class="col-xs-12 connectedSortable">
								<div id='jqxGridPurchase_methodToolbar' class='grid-toolbar'>
									<p>
										<button class="btn btn-xs btn-primary btn-flat" id="jqxGridInvoiceItem" type="button">Add New Item</button> 
									</p>
								</div>
		...
		<div class="row">
			<div style="float:right; ">
				<div class="btn-group">
					<button class="btn btn-default" onclick="printPreview('PurchaseInvoice')">Print</button>&nbsp
...
<div id="jqxPopupWindowInvoiceitem">
	...
	<div class="form_fields_area row">
		<div class="col-md-12">
			...
			<div class="row form-group">
				...
				<div class="col-md-4"><input type="number" id='qty' class='form-control input-sm' name='qty'></div>
				<div class="col-md-2"><label for='price'><?php echo lang('price')?></label></div>
				<div class="col-md-4"><input type="number" id='price' class='form-control input-sm' name='price'></div>
			</div>
			<div class="row form-group" id="add_item_vat" hidden>
				<div class="col-md-2"><label for='vat'><?php echo lang('vat')?></label></div>
				<div class="col-md-4"><input type="number" id='vat' class='form-control input-sm' name='vat'></div>
			</div>
			<div class="row form-group">
				<div class="col-md-2"><label for='disc'><?php echo lang('disc')?>(%)</label></div>
				<div class="col-md-4"><input type="number" id='disc' class='form-control input-sm' name='disc'></div>
		...
		<div class="row form-group">
			<div class="col-md-12">
				...
				<div class="btn-group btn-group-sm pull-right">
					<button type="button" class="btn btn-link btn-flat" id="jqxGridInvoiceAddLocalPart" >Add New Local</button> 
					<button type="button" class="btn btn-success btn-flat" id="jqxPurchase_methodSubmitButton"><?php echo lang('general_add'); ?></button>
					<button type="button" class="btn btn-default btn-flat" id="jqxPurchase_methodCancelButton"><?php echo lang('general_cancel'); ?></button>
				</div>
			</div>
		</div>
...
<div id="jqxPopupWindowAddLocalPart">
	<div>Add Local Parts</div>
	<div class="row">
		<div class="col-md-12">
			<form id="form-add_local_parts">
				<div class="row">
					<div class="col-md-11">
						...
						<div class="row">
							...
							<div class="col-md-9">
								<input id="local_parts-partname" name="partname" class="form-control" />
								<!-- <div id="local_parts-partname" name="partname" class="form-control"></div> -->
							</div>
						</div>
						<div class="row">
							<!-- <div class="col-md-3">Quantity</div> -->
							<!-- <div class="col-md-3"><input id="local_parts-quantity" name="quantity" type="text" class="form-control"></div> -->
							<div class="col-md-3">Price</div>
							<div class="col-md-6"><input id="local_parts-price" name="price" type="text" class="form-control"></div>
						</div>
						<!-- <div class="row"> -->
							<!-- <div class="col-md-3">VAT</div> -->
							<!-- <div class="col-md-3"><input id="local_parts-vat" name="vat" type="text" class="form-control"></div> -->
							<!-- </div> -->

							<div class="pull-right">
								<button type="button" class="btn btn-sm btn-primary" id="local_parts-submit">Add</button>
							</div>
						</div>
					</div>
				</form>
	...
	<script language="javascript" type="text/javascript">
		...
		$(function(){
			...
			var purchase_invoicesDataSource =
			{
				...
				datafields: [
				{ name: 'id', type: 'number' },
				{ name: 'purchase_invoice_serial', type: 'number' },
			...
			$("#jqxGridPurchase_invoice").jqxGrid({
				...
				{
					text: 'Action', datafield: '', width:75, sortable:false,filterable:false, pinned:true, editable: false, align: 'center' , cellsalign: 'center', cellclassname: 'grid-column-center', 
					cellsrenderer: function (index) {
						var e = '<a href="javascript:void(0)" onclick="editPurchase_invoiceRecord(' + index + '); return false;" title="View"><i class="fa fa-list-alt"></i></a>';
						return '<div style="text-align: left; margin-top: 8px; margin-left: 7px">' + e + '</div>';

					}
				},
				{ text: '<?php echo lang("purchase_invoice_serial"); ?>',datafield: 'purchase_invoice_serial',width: 150,filterable: true,renderer: gridColumnsRenderer },
			...
			$(document).on('click','#jqxGridPurchase_invoiceInsert', function () {
				...
				$.post('<?php echo site_url('purchase_invoices/get_purchase_invoice_serial'); ?>', function(r) {
					// console.log(r.purchase_invoice_serial);
					$('#purchase_invoice_serial').val(r.purchase_invoice_serial);
				},'json');
	...
	$("#local_parts-submit").on('click', function() {
		var data = getFormData('form-add_local_parts');

		$.post('<?php echo site_url('purchase_invoices/new_local_part') ?>', data, function(r) {
			// $("#part_id").jqxComboBox('insertAt',{label: data.partname, value: r, price: data.price } );
			if(r.success){

				$('#form-add_local_parts')[0].reset();
				$('#jqxPopupWindowAddLocalPart').jqxWindow('close');

				$('#part_id').jqxComboBox('clear');
				// $('#part_id').val(r.success);
			}
	...
	/*var localpartsSource =
	{
		...
		url: '<?php echo site_url('purchase_invoices/get_localparts')?>'
	};

	localpartsAdapter = new $.jqx.dataAdapter(localpartsSource);

	$("#local_parts-partname").jqxComboBox({
		theme: theme,
		width: 195,
		height: 25,
		selectionMode: 'dropDownList',
...
function editPurchase_invoiceRecord(index){
	...
	if (row) {
		$('#jqxGridPurchase_method').jqxGrid('clear');
		$('#purchase_invoices_id').val(row.id);
		$('#purchase_invoice_serial').val(row.purchase_invoice_serial);
		...
		$('#vat').val(row.vat);
		$('#vatamount').val(row.vatamount);
		$('#netamount').val(row.netamount);

		$.post("<?php echo site_url('admin/purchase_methods/get_purchase_method')?>",{id:row.id},function(data){
...
function reset_form_purchase_invoices(){
	$('#purchase_invoices_id, #purchase_invoice_serial').val('');
...
<script language="javascript" type="text/javascript">
	...
	$(function(){
		...
		$("#jqxGridPurchase_method").jqxGrid({
			...
			{ text: 'Part Code',datafield: 'part_code', columntype: 'dropdownlist', width: 150,filterable: true,renderer: gridColumnsRenderer, editable: false, },
		...
		var partsource = {
			url : '<?php echo site_url("admin/purchase_invoices/get_spareparts_lists"); ?>',
			datatype: 'json',
			data: { type: 'local' },
			datafields: [
			{ name: 'id', type: 'number' },
...
$(document).on('click','#jqxGridInvoiceAddLocalPart', function () { 
		// Job_form_table.jqxGrid('clear');
		openPopupWindow('jqxPopupWindowAddLocalPart', '<?php echo lang("general_add")  . "&nbsp;" .  lang("item"); ?>');
	});
	...
	$("#jqxPopupWindowInvoiceitem").jqxWindow({ 
		...
		modalOpacity: 0.7,
		showCollapseButton: false,
		cancelButton: $('#jqxPurchase_methodCancelButton')
	});

	$("#jqxPopupWindowAddLocalPart").jqxWindow({ 
		theme: theme,
		width: '30%',
		maxWidth: '75%',
		height: '20%',  
	...
	$('#jqxPurchase_methodSubmitButton').click(function(){
		var partItem = $('#part_id').jqxComboBox('getSelectedItem');
		var qty = $('#qty').val();
		var price = $('#price').val();
		var disc = $('#disc').val();
		var bin = $('#bin').val();
		...
		var datarow = {
			'part_id'           :partItem.value,
			'part_code'         :partItem.originalItem.part_code,
			'description'       :partItem.originalItem.name,
			'qty'               :qty,
			'price'             :price,
			'disc'              :disc,
			'bin'               :bin,
	...
	function printPreview(type, id = null) {
		id = $('#purchase_invoices_id').val();

		var url = '<?php echo site_url('admin/purchase_invoices/print_preview?id=') ?>' + id + '&type=' + type;

		myWindow = window.open(url, '_BLANK', "height=900,width=1300");
