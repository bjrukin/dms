		...
		<div class="form_fields_area">
			...
			<table class="form-table">
				<tr>
					...
					<td><div id='call_status_thirty' name='call_status'></div></td>
				</tr>
				<tr class="inq_detail" style="display: none">
					<?php /*<td><label for='product_feedback'><?php echo lang('product_feedback')?></label></td>
				...
				<tr class="inq_detail" style="display: none">
					...
					<td><div id='registration_number' class='array_decisions' name='registration_number'></div></td>
				</tr>
				<tr class="inq_detail" style="display: none">
					<td><label for='call_for_service'><?php echo lang('call_for_service')?></label></td>
					<td><div id='call_for_service' class='array_decisions' name='call_for_service'></div></td>
				</tr>
				<tr class="inq_detail" style="display: none">
					<td><label for='total_score'><?php echo lang('total_score')?></label></td>
					<td><input type="text" id='total_score_thirty' name='total_score' readonly></td>
				</tr>
				<?php /*<tr>
				...
				<?php /*<tr class="inq_detail" style="display: none">
				...
				<tr class="inq_detail" style="display: none">
					...
					<td><div id='remarks_thirty' class='array_ccd_remarks' name='remarks'></div></td>
	...
	<script language="javascript" type="text/javascript">
		...
		$(function(){
			...
			$("#call_status_thirty").jqxComboBox({
				theme: theme,
				width: 195,
				height: 25,
				selectionMode: 'dropDownList',
			...
			$("#call_status_thirty").bind('select', function (event) {
				if (!event.args)
					return;

				call_status = $("#call_status_thirty").jqxComboBox('val');
			...
			var ccd_thirtydaysDataSource =
			{
				...
				{ name: 'call_for_service', type: 'string' },
				{ name: 'registration_number', type: 'string' },
				
				],
				url: '<?php echo site_url("admin/ccd_thirtydays/json"); ?>',
	...
	function editCcd_thirtydayRecord(index){
		...
		$('#call_status_thirty').jqxComboBox('clearSelection');
		$('#bluebook_copy').jqxComboBox('clearSelection');
		$('#green_sticker').jqxComboBox('clearSelection');
		$('#remarks_thirty').jqxComboBox('clearSelection');
		$('#registration_number').jqxComboBox('clearSelection');
		$('#call_for_service').jqxComboBox('clearSelection');

		if (row) {
			$('#ccd_thirtydays_id').val(row.id);
			$('#customer_name').html(row.full_name);
			// $('#call_status_thirty').jqxComboBox('val',row.call_status);
			...
			// $('#remarks_thirty').jqxComboBox('val',row.remarks);
			// $('#registration_number').jqxComboBox('val',row.registration_number);
			// $('#call_for_service').jqxComboBox('val',row.call_for_service);
			// $('#payment_receipts').jqxComboBox('val',row.remarks);
			// $('#voc').val(row.voc);
			// $('#total_score_thirty').val(row.voc);
...
<script type="text/javascript">
	$(document).on('change','.array_decisions', function () {
		var green_sticker = $('#green_sticker').jqxComboBox('val');
		var registration_number = $('#registration_number').jqxComboBox('val');
		var call_for_service = $('#call_for_service').jqxComboBox('val');
		
		var total = 0;

		if(green_sticker == 'Yes'){
			total += 33;
		}
		if(registration_number == 'Yes'){
			total += 33;
		}
		if(call_for_service == 'Yes'){
			total += 34;
		}

		$('#total_score_thirty').val(total);
	});
</script>