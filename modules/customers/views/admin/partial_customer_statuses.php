...
<div id="jqxPopupWindowCustomerStatus">
	...
	<div class="form_fields_area">
		...
		<table class="form-table">
			...
			<tr id="identitydocumentupload" hidden>
				<td><label >Document Upload<span class='mandatory'>*</span></label></td>
				<td><div id="jqxFileUpload"></div></td>
				<td colspan="2">(Notes: file must be image file of identity proof i.e. license, citizenship, office document etc)</td><input type="hidden" name="uploadeddocument" id="uploadeddocument">
			</tr>
	...
	<div id='jqxGridCustomer_statusToolbar' class='grid-toolbar'>
		<?php if($customer_info->status_name != 'Retail' && $customer_info->status_name != 'Closed' && $customer_info->status_name != 'Cancel'){?>
	...
	<script language="javascript" type="text/javascript">
		var Lateststatus = '<?php echo $customer_info->status_name ?>';

		var customer_statuses = function() {
			$('#jqxFileUpload').jqxFileUpload({ width: 300, uploadUrl: '<?php echo site_url('admin/customers/documentUpload') ?>', fileInputName: 'userfile' });
			$('#jqxFileUpload').on('uploadEnd', function (event) {
				console.log(event);
			    var args = event.args;
			    var fileName = args.response;
			    var serverResponce = args.response;
			    if(serverResponce=='105'){
			    	alert("File Upload Failed. Try Again.");
			    }else{
			    	$('#uploadeddocument').val(fileName);
			    }
			});
			...
			var __actualStatusName = '<?php echo $customer_info->actual_status_name ?>';
			// if('<?php ?>')
			if(__actualStatusName == 'Booked'){
				$("#status_id").jqxComboBox('removeItem', "1" ); 
				$("#status_id").jqxComboBox('removeItem', "2" ); 
			...
			$("#status_id").select('bind', function (event) {
				...
				if (status_id == <?php echo STATUS_PENDING; ?>) {
					...
					var SubStatusDataSource  = {
						...
						data:{ id : 1 , status_name: Lateststatus },
						async: false,
						cache: true
					}	
					...
					if('<?php echo  $dealer_id; ?>' != '<?php echo TAXI_SALE;  ?>'){
						$('#identitydocumentupload').show();
					}	
					var SubStatusDataSource  = {
						url : '<?php echo site_url("admin/customers/get_customer_inquiry_sub_statuses"); ?>',
						datatype: 'json',
					...
					var SubStatusDataSource  = {
						...
						data:{ id : 2 , status_name: Lateststatus },
						async: false,
						cache: true
					}
				}
					...
					var SubStatusDataSource  = {
						...
						data:{ id : 3 , status_name: Lateststatus },
						async: false,
						cache: true
					}
				}
    ...
    $("#sub_status_id").select('bind', function (event) {
    	...
    	if (sub_status_id == <?php echo STATUS_QUOTATION_ISSUED; ?>) {		// STATUS_QUOTATION_ISSUED
    		...
    		$('.bank-row').show();
				...
				$("#bank_id").jqxComboBox('removeItem', '99999');
	...
	{ input: '#uploadeddocument', message: 'Required', action: 'blur', 
	    rule: function(input) {
	    	var status_id = $('#status_id').val();
	    	if('<?php echo  $dealer_id; ?>' == '<?php echo TAXI_SALE;  ?>'){
				return true;
			}	
	    	if(status_id == <?php echo STATUS_BOOKED; ?> )
	    	{    			
	    		val = $('#uploadeddocument').val();
	    		return (val == '' || val == null || val == 0) ? false: true;
	    	}
	    	else
	    	{
