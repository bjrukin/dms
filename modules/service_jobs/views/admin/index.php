...
<script language="javascript" type="text/javascript">
    ...
    $('#form-service_jobs').jqxValidator({
        ...
        rules: [
			...
			{ input: '#job_code', message: 'Job Code must be unique', action: 'blur', 
				rule: function(input, commit) {
					val_id = $('#service_jobs_id').val();

					if(val_id){
						return true;
					}else{

						$.post('<?php echo site_url('admin/service_jobs/checkJobCode')?>',{val:val},function(data){
...
<script type="text/javascript">
	$(function(){
		var grid_modelwiseDataSource = {
			...
			// url: '<?php //echo site_url("admin/service_jobs/json"); ?>',
			url: '<?php echo site_url("admin/service_jobs/get_all_vehicles"); ?>',
			pagesize: defaultPageSize,
			root: 'rows',
			id : 'id',
			cache: true,
