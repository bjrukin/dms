...
// Display Page
    ...
    public function json_retail_request_list(){
        ...
        search_params();
        $this->db->where($where);
        $total=$this->dealer_order_model->find_count(array('cancel_date'=>NULL,'credit_control_approval'=>1,'is_ktm_dealer'=>1));

        paging('id');

        search_params(); 
        $this->db->where($where);
        $rows=$this->dealer_order_model->findAll(array('cancel_date'=>NULL,'credit_control_approval'=>1,'is_ktm_dealer'=>1));
...
// Display Page
    ...
    public function json_dealer_incharge(){
        ...
        search_params();
        $total=$this->dealer_order_model->find_count(array('cancel_date'=>NULL,'credit_control_approval'=>1,'is_ktm_dealer'=>0));

        paging('id');

        search_params(); 
        $rows=$this->dealer_order_model->findAll(array('cancel_date'=>NULL,'credit_control_approval'=>1,'is_ktm_dealer'=>0));
...
//load upload library
    ...
    public function check_stock_availability()//dealer_order/admin
    {
        ...
        if($is_ktm_dealer == 1)
        {
            $this->db->where('(vehicle_main_id IS NULL AND cancel_date IS NULL AND credit_control_approval = 1 AND (in_stock_remarks = 1 OR in_stock_remarks = 2) AND is_ktm_dealer = 1)');
        ...
        // Get Transit
            ...
            if($current_stock_count_ktm > $current_order_count)
            {
                $success = 'instock';
            }
            elseif($current_stock_count_ktm + $current_stock_count_out_of_ktm > $current_order_count)
            {
                $success = 'outside_ktm';
            }
            elseif($current_stock_count_ktm + $current_stock_count_out_of_ktm + $transit_not > $current_order_count)
            {
                $success = 'transit';
            }
            else
        ...
        {
            $this->db->where('(vehicle_main_id IS NULL AND cancel_date IS NULL AND credit_control_approval = 1 AND (in_stock_remarks = 1 OR in_stock_remarks = 2)  AND is_ktm_dealer = 0)');
            $current_order_count = $this->dealer_order_model->find_count(array('vehicle_id'=>$vehicle_id,'variant_id'=>$variant_id,'color_id'=>$color_id,'year'=> $mfg_year));
            ...
            if($current_stock_count > $current_order_count)
            {
                $success = 'instock';
            }
            elseif( $current_stock_count + $transit_not > $current_order_count )
            {
                $success = 'transit';
            }
            else
