...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1><?php echo lang('dealer_orders_list'); ?></h1>
        <ol class="breadcrumb">
            <li><a href="#">Home</a></li>
...
<div id="jqxPopupWindowDealer_order">
    ...
    <div class="form_fields_area">
        ...
        <input type = "hidden" name = "in_stock_ktm" id = "in_stock_ktm"/>
        ...
        <table class="form-table">
            ...
            <?php if($this->session->userdata('employee')['dealer_id'] == 1 || $this->session->userdata('employee')['dealer_id'] == 2):?>
                <tr>
                    <td><label for="delivery_week">Delivery Week</label></td>
                    <td><div id="delivery_week" name="delivery_day"></div></td>
                </tr>
            <?php endif; ?>
            <tr id="ktm_error_div" style="display: none">
                <td></td>
                <td><span style="color:#f00;font-size : 20px; ">Not Stock in Ktm</span></td>
            </tr>
            <tr>
...
<div id="jqxPopupWindowChange_color">
    ...
    <div class="form_fields_area">
        ...
        <tr>
            <td><label for='year'><?php echo "Year" ?></label></td>
            <td><input type="text" class='text_input' name="year" id="year_cc" readonly="true"></td>
        </tr>
        <tr>
            <td><label for="order_month"><?php echo "Order Month" ?></label></td>
...
<div id="jqxPopupWindowgrn_add">
    ...
    <div class="form_fields_area">
        ...
        <table class="form-table app-table">
            <tr>
                <td> <label><span>GRN Date</span></label> </td>
                <td> <div id="grn_date" name="grn_date"></div> </td>
            </tr>
            <tr>
            ...
            <tr>
                <th colspan="3">
                    <button type="button" class="btn btn-success btn-md btn-flat" id="jqxGrn_addSubmitButton"><?php echo lang('general_save'); ?></button>
...
<script language="javascript" type="text/javascript">
    ...
    $(function () {
      ...
      var ktm_dealer_check = <?php echo ($ktm_dealer)?$ktm_dealer:'null' ?>;
      ...
      if(dealer_id == 1 || dealer_id == 2)
      {
        ...
        var k = 0;
        for (var i = 0; i < deliveryWeek.length; i++) {
            var row = {};
            row["delivery_week"] = deliveryWeek[k];
            row["delivery_days"] = delivery_days[k];
            data[i] = row;
            k++;
        }
        ...
        var deliveryAdapter = new $.jqx.dataAdapter(Deliverysource);

        $("#delivery_week").jqxComboBox({ 
            source: deliveryAdapter, 
            width: '200px', 
            height: '25px',
            placeHolder : 'Select a value',
            displayMember : 'delivery_week',
            valueMember : 'delivery_days'
        });
    }
    ...
    $("#delivery_week").bind('select', function (event) {
        ...
        color_id        = $("#color_id").jqxComboBox('val');
        year            = $("#year").val();
        delivery_week   = $("#delivery_week").jqxComboBox('val');

        $.post("<?php echo site_url('dealer_orders/check_ktm_stock_available')?>",{vehicle_id:vehicle_id , variant_id:variant_id, color_id : color_id, year:year,delivery_week:delivery_week},function(result)
        {
    ...
    {
        if(ktm_dealer_check){
            if (data.in_stock_remarks == 1 && data.stock_in_ktm == 1) {
                return 'cls-green';
            }
            else if(data.stock_in_ktm == 0 && data.in_stock_remarks == 1)
            {
                return 'cls-blue';
            }
    ...
    var show_hide = '';
    if((is_admin == 1 || is_sales_head == 1) || (showroom_incharge && (dealer_id == 1 || dealer_id == 2)))
    {
        show_hide = false;
    }
    else
    ...
    $("#jqxGridDealer_order").jqxGrid({
        ...
        {text: '<?php echo lang("stock_status");?>', datafield: 'stock_status', width: 100, filterable: true, renderer: gridColumnsRenderer, cellclassname : cellclassname},
        {text: '<?php echo lang("stock_in_ktm_status");?>', datafield: 'stock_in_ktm_status', width: 100, filterable: true, renderer: gridColumnsRenderer, cellclassname : cellclassname ,  hidden: show_hide },
        ...
        {text: '<?php echo lang("credit_control_approve_date"); ?>', datafield: 'credit_control_approve_date', width: 90, filterable: true, renderer: gridColumnsRenderer, columntype: 'date', filtertype: 'range', cellsformat: formatString_yyyy_MM_dd , cellclassname : cellclassname},
        ...
        {text: '<?php echo lang("customer_retail_date"); ?>', datafield: 'customer_retail_date', width: 90, filterable: true, renderer: gridColumnsRenderer, columntype: 'date', filtertype: 'range', cellsformat: formatString_yyyy_MM_dd , cellclassname : cellclassname},
        {text: '<?php echo lang("retail_nepali_month"); ?>', datafield: 'retail_nepali_month', width: 150, filterable: true, renderer: gridColumnsRenderer , cellclassname : cellclassname},
        ],
...
$(document).on('click', '#jqxGridDealer_orderInsert', function () {
    openPopupWindow('jqxPopupWindowDealer_order', '<?php echo lang("general_add") . "&nbsp;" . $header; ?>');
     ...
     $('#form-Grn_add').jqxValidator({
        hintType: 'label',
        animationDuration: 500,
        rules: [
        { input: '#grn_date', message: 'Required', action: 'blur', 
        rule: function(input) {
            val = $('#grn_date').val();
            return (val == '' || val == null || val == 0) ? false: true; 
        } 
     ...
     $("#jqxGrn_addSubmitButton").on('click', function () {
        ...
        $('#form-Grn_add').jqxValidator('validate', validationResult);
     ...
     function credit_edit(index){
        var row = $("#jqxGridDealer_order").jqxGrid('getrowdata', index);
        var admin = '<?php if(is_admin()) { echo "1"; } ?>';
        if (row.credit_control_approval != 1 || admin == 1) 
        {
        ...
        {
            alert('Edit disabled (Payment Approved)');
        }
    }
    ...
    function saveDamageRecord(index){
        ...
        $('#jqxPopupWindowDamage_entry').block({ 
            message: '<span>Processing your request. Please be patient.</span>',
            css: { 
                width                   : '75%',
                border                  : 'none', 
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#jqxPopupWindowDamage_entry').unblock();
            }
        });
    }
