...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		if ( $query->num_rows() != 0 && $this->verify_password($password, $row->pass) ) {
			...
			$data = array(
				...
				'email' => $row->email,
				'loggedin' => TRUE,
				'password_change_date' => $row->password_change_date,
				'password_change_status' => $row->password_change_status,
			);
	 ...
	 * @return bool Update fails/succeeds
	 */
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE, $fullname = false, $password_change = false) {
		...
		if($password_change){
			$data['password_change_status'] = false;
			$data['password_change_date'] = date('Y-m-d');
		}
		// print_r($data);
		// print_r($password_change);
		// exit;
