...
class Users extends Project_Controller
{
	...
	public function json()
	{
		...
		$this->user_model->_table = 'view_aauth_users';
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            if ($this->input->post('password')){
                ...
                $user_id = $success = $this->aauth->update_user($id, $email, $password, $username, $fullname );
            } else {
    ...
    public function get_user_groups()
    {
        ...
        $this->db->where_not_in('group_id', array(1));
    ...
    public function make_pw_change()
    {
        $user_id = $this->input->post('id');
        $data = array('password_change_status' => true);
        $where['id'] = $user_id;
        $this->db->where($where);
        $this->db->set($data)->update('aauth_users');
