...
<div class="form_fields_area">
    ...
    <table class="form-table">
       ...
       <tr>
        ...
        <td><input id='password' class='text_input' type='password' name='password'></td>
    </tr>
    <tr>
     ...
     { text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
     {
        ...
        cellsrenderer: function (index) {
           ...
           if(row.id > '100') {
             ...
             p = '<a href="#" onclick="password_change_notify('+row.id+')" title="Change Password"><i class="fa fa-lock"></i></a>';

             return '<div style="text-align: center; margin-top: 8px; font-size:14px">' + e + '&nbsp;'+ resetPasswd + '&nbsp;' + lock + '&nbsp;' + resetLogin + '&nbsp;' + verify + '&nbsp;' + g + ' ' + p +'</div>';
         } else {
...
$(document).on('click','#jqxGridUserInsert', function () { 
    ...
    $('#form-users').jqxValidator({
        ...
        { input: '#password', message: 'Password must be between 8 and 32 characters!', action: 'blur',
        rule: function(input, commit) {
            ...
            if (val == '' && mode == 'update') {
                ...
                return (val.length < 8 || val.length > 21) ? false : true;
            }
        }
    },
...
function password_change_notify(id){
	if(confirm("Are you sure want to make this user change password")){
		$.post('<?php echo site_url('admin/users/make_pw_change')?>',{id:id},function(){
			alert('success');
		});
	}
}
...
function editUserRecord(index){
    ...
    if (row) {
        ...
        $.post('<?php echo site_url('users/get_user_groups') ?>',{id:row.id},function(result){
            $.each(result.involved_groups,function(i,v)
            {
                $('#group_'+v.group_id).attr('checked',true);
            });
        },'JSON');
    ...
    if (row) {
        ...
        $.ajax({
            ...
            $.each(not_involved_groups,function(index,group) {
                ...
                if(i == -1) {
                    var item = '<li class="item unselected-item" id="allgroup_' + group.id + '" data-not_involved_group_id="' + group.id + '">' + group.name + '</li>';
            ...
            $.each(involved_groups,function(index,group) {
                ...
                if(i == -1) {
                    var item = '<li class="item selected-item" id="involvedgroups' + group.group_id + '" data-involved_group_id="' + group.group_id + '">' + group.group + '</li>';
                    $("#involved_groups").append(item);
                }
            });
        }
